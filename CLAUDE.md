...
85. **Six Learning Loop deepening features:**
    ...
    - **HOLD outcome tracking:** `_resolve_hold_outcomes()` writes `virtual_hold_miss`/`virtual_hold_correct` (±8% band). Emits `⚠HOLD_TOO_PASSIVE` when miss rate >50% n≥10. HOLDs are appended to `logRecsToLearningLoop()` after the UI filter strips them (`_holdRecs`, analysis.js); `POST /api/learning/log` dedupes to one HOLD per ticker per 7d (`_HOLD_DEDUP_DAYS`), returning the existing id with `deduped:true`.
