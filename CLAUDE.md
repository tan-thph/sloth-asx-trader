...
69. **Both Day Trading sub-strategies gate the final "is this a setup" decision on a confirm-signal COUNT, not a blunt bypass-all toggle.** This replaced two different all-or-nothing testing toggles after they made it impossible to find zero-setup days were actually working-as-designed (a hardcoded combination requirement, not a bug):
    - **Swing** (`js/day-trading-analysis.js:177-230`, `_dtBuildRecs`): Signal #1 (BB reclaim) is automatic — guaranteed by the pre-filter's `maxBbPctB` gate. Signals #2–#5 (RSI oversold, Volume Z-score, Fib retracement zone, OBV rising) each independently fire or don't; `aiParams.minConfirms` (default **2**, range 1–4, editable in Day Trading → Rules → Entry Signal Thresholds) sets how many of those 4 must fire. This was previously a hardcoded literal `2` with no UI control at all — a user could max out every individual signal threshold (RSI/VolZ/Fib range) and still get 0 setups with no way to know the combination COUNT itself was the blocker, since there's still no per-signal rejection breakdown at this stage (only the pre-filter has one, surfaced as "Rejected — BB:70 ADV:9 SMA:47 ADX:0").
    - **Intraday** (`js/intraday-strategy.js:49-73`, `_buildIntradayRecs`): the 4 confirm-signals are VWAP-below-threshold, RSI-below-ceiling, volume rising, and price-above-today's-open. `params.minConfirms` (default **2**, range 1–4, Day Trading → Intraday tab → Rules) sets how many must fire. **Replaces the old binary `extremeMode`/`requireVolRising` toggles entirely** — `extremeMode` used to bypass the entry-window, VWAP, RSI, and R:R gates ALL AT ONCE with zero granularity (and `requireVolRising` was a separate, redundant boolean for what is now just one of the 4 counted signals). Entry-window timing (10:45–15:00 AEST) is deliberately kept OUT of the confirm-count — it's a hard market-structure constraint, not a technical confirmation — and is instead controlled by a narrowly-scoped `params.ignoreEntryWindow` (testing-only, explicitly labelled, default false). `minRrRatio` is now **always enforced** for both Swing and Intraday (previously bypassable by `extremeMode`) — loosen the threshold value itself for testing rather than expecting a bypass flag to exist.
    - If you add a new confirm-signal to either strategy, it changes the denominator ("of 4") — update both the UI label and this gotcha.
