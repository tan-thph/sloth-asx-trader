...
| `js/prompts.js` | All Claude system prompts (`ANALYSIS_SYSTEM_PROMPT`, `MACRO_SYSTEM_PROMPT`, `MORNING_BRIEFING_SYSTEM_PROMPT`, day-trade prompts, analyst/PM prompts). Rule 3 references `stopAtrMultiple` from ACTIVE RULE OVERRIDES (regime-aware, injected by `analysis.js`). Rule 4 sets qty=0 (quant engine sizes post-response: `computeTradeParams()` for BUY/TOP_UP; the SELL/TRIM block in `analysis.js` sizes exits — SELL = full holding, TRIM = `weightGuidance` Reduce-range midpoint, default 35%). `scenarios` field is optional. `PROMPT_VERSION = '2026-06-v9'`. |
...
| `js/charts.js` | Canvas helpers. `_fitCanvas(canvas)` — sets `width`/`height` at `devicePixelRatio` for crisp rendering on all screens; used by every draw function. Debounced `resize`/`orientationchange` handler redraws open charts when viewport *width* changes (ignores height-only shifts from mobile toolbars). |
