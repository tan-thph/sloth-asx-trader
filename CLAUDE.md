...
state.settings.corrBlockThreshold: number — hard-block BUY recs when |ρ| with any holding ≥ this threshold (default 0.85). Soft gate fires 10pp below (−30% size). Configurable in Settings → Telegram Alerts card. *(Sprint 49)*
...
- `GET /api/db/git-status` — returns `{ok, last_committed, has_uncommitted}`. `last_committed` is the ISO timestamp of the last git commit touching `asx_trader.db`; `has_uncommitted` is true when the file has uncommitted changes. Returns `{ok: false, error}` when git is not installed. Displayed in Settings → App Info. *(Sprint 49)*
