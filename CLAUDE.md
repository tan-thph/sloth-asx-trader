...
| `js/analysis.js` | Portfolio analysis orchestration — fetches signals, builds prompt, calls Claude, post-processes through quant engine + validator + regime modifiers. `portfolioJson` uses `livePrice = state.liveSignals[ticker]?.current_price ?? h.currentPrice` for all price-derived fields (currentPrice, value, unrealisedPnl, unrealisedPnlPct, weight) — ensures SELL/TRIM `priceRange` is anchored to fresh signal price, not stale portfolio cache. |
