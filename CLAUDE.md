...
| `js/prompts.js` | All system prompts. `PROMPT_VERSION = '2026-07-v19'`. `scenarios` required non-HOLD. `bullCase` required BUY/TOP_UP. `bearCase` required SELL/TRIM and BUY/TOP_UP ≥0.70 confidence. `invalidationCondition` required non-HOLD. `primary_entry_driver` required BUY/TOP_UP. Top-level response now carries `portfolioSynthesis` (required, Section 3B book-level PM view — concentration/rotation/cash%/what-changed) and `deferrals[]` (anti-churn/timing holdbacks, split out of `dataGaps[]` which is now genuine missing-data only). HOLD + SELL/TRIM recs with a HOLDING_CONTEXT block must lead with the original EntryDriver's fundamental status and cite an entry-vs-now signal delta (Section 2B + Rule 5). |
