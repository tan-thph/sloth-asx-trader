...
| `health_digest.py` | Daily system-health digest (DB integrity, backup freshness+validity, news-scanner health, yfinance failure volume). Notification centre always; Telegram only on warn/error. Scheduler started per entry point (`asx_server.py __main__` + `waitress_server.py`), NOT module-level (test imports). On-demand: `GET /api/system/health-digest` (`?notify=1` delivers, bypassing the once-per-day guard). |
...
| `routes/news.py` | `/api/news/*`, `/api/groq/models`, `/api/google/models`, `/api/sbc-mode`, `/api/system/gpu`, `/api/system/health-digest`, `/api/ollama/start` | RSS scanner + provider management + health digest. |
...
| `js/prompts.js` | All system prompts. `PROMPT_VERSION = '2026-07-v22'`. `scenarios` required non-HOLD. `bullCase` required BUY/TOP_UP. `bearCase` required SELL/TRIM and BUY/TOP_UP ≥0.70 confidence. `invalidationCondition` required non-HOLD. `primary_entry_driver` required BUY/TOP_UP. Response shape is `{recs, summary, dataGaps}` — anti-churn/timing holdbacks go in `dataGaps[]` (no separate `deferrals[]`). HOLD + SELL/TRIM recs with a HOLDING_CONTEXT block must lead with the original EntryDriver's fundamental status and cite the block's own **computed** entry-vs-now signal delta (Section 2B + Rule 5) — `analysis.js` now calls `computeThesisDrift()` when building `HOLDING_CONTEXT` itself (not just post-hoc for logging), so the block carries a real `Delta: RSI 42→68 (verdict)` string instead of leaving Claude to infer "now" from elsewhere in the prompt. **Book-level "portfolio synthesis" is NOT LLM-emitted** — it is computed deterministically in `analysis.js` (top-sector/top-name concentration + cash %, plus a compact changes-since-last-review tail) and rendered as the "📊 Book snapshot" panel. The verbose `CHANGES SINCE LAST REVIEW` block (engine-computed price/regime/stance diff vs the prior run's persisted `state.analysisLastSummary.review` snapshot — `{regime, prices, recs}`) is still injected into the prompt so recs focus on new information. |
...
- `GET /api/learning/hold-recs?limit=N` — list of logged HOLD recs (default 100, cap 200) with each one's `virtual_outcome` (30-day passivity verdict) or a `days_until_review` countdown. HOLDs are NOT in client rec state (analysis.js strips them), so this is the only surface for them — feeds the Learning-page "HOLD Decisions" card.
...
- `GET /api/learning/sell-outcomes` / `buy-outcomes` — SELL/TRIM (resp. BUY/TOP_UP) events. Both accept `?days=N` (Learning-page master window, `_days_window_sql()`): when set, filters `timestamp` to last N days and relaxes the count cap so the window bounds the set. Aggregate/stat endpoints deliberately ignore `days` (full history is the learning signal)
- `GET /api/learning/rule-override-efficacy` — per failed-rule-code WR/avg-P&L of breached-and-**closed** trades vs baseline; verdict `too_strict`/`validated`/`inconclusive` (Wilson-CI + `min_n` gated). Splits `closed_n`/`open_n`/`skipped_n`; open BUY/TOP_UP carry NO realized P&L. Returns a per-rule `events[]` drill-down (id, ticker, action, state, realized P&L, entry_price)
...
58. **Intraday stops/targets are ATR-based, not fixed-%.** `stop = entry − 1.5 × atr_5m`, `target = vwap + 2 × atr_5m` (aspirational take-profit). Falls back to fixed-% only when `atr_5m` null. **The R:R gate uses the HONEST reversion R:R** (`_intradayReversionRR` = reward-to-VWAP ÷ risk), NOT reward-to-extended-target — the old to-target gate was inflated by the ATR-overshoot term and passed almost everything. `rec.rrRatio` now carries the reversion value (gated on `minRrRatio`, default 1.0 for intraday — lower than swing's 2.0 because it's a conservative reversion basis); `rec.rrExtended` carries the to-target value for display. Falls back to reward-to-target when VWAP isn't above entry. **Kelly `winProb` also uses the ridge model** (same wiring as swing): `_dtImpliedWinProb(E[R], rrExtended)` off `_dtModel.intraday` when it's a `ridge` model with `n_samples ≥ 40`, else the `score/100×0.85` heuristic. The prediction feature vector MUST mirror the intraday snapshot builder (`executeIntraday` in `pages/day-trading.js`): daily `liveSignals` + `{intraday_rsi, vwap_pct, volume_acceleration:null, score}`. `rrExtended` (not the reversion `rrRatio`) is the correct b for the inversion since the model's r_multiple is in intraday-stop R-units. ML-sized recs carry `_mlSized/_mlWinProb/_mlExpectedR` + a `🤖 p%` chip.
...
85. **Six Learning Loop deepening features:**
    ...
    - **HOLD outcome tracking (redesigned 2026-07-09):** a HOLD is a decision to keep an existing **long** holding, so `_resolve_hold_outcomes()` judges it **direction-aware, endpoint-based, over a bounded forward window** — NOT the old flat-8% peak-in-an-unbounded-`entry→today`-window rule (that grew miss-probability with resolution lag, counted transient spikes, and scored an 8% *rally* on a correctly-held long as a "miss" — biasing the nudge toward over-trading). Now: fetch a bounded window (`history(start, end=entry+45d)`), take the **endpoint** return over the first `_HOLD_HORIZON_DAYS` (20) trading bars, and flag `virtual_hold_miss` only when that endpoint return `≤ −threshold` where **threshold is vol-scaled off the entry snapshot's `atr_pct`** (`_hold_move_threshold` = `K·(atr/100)·√H`, floored 5%, capped 20%; falls back to the flat `move_threshold` when `atr_pct` absent). Upside/flat = `virtual_hold_correct`; a transient dip that recovers by the horizon is NOT a miss (persistence required); `<10` forward bars → `virtual_open`. The `⚠HOLD_TOO_PASSIVE` nudge is now **CI-gated like every other behaviour-changing nudge** (`_compute_hold_outcome_nudge`): fires only when `n≥_MIN_NUDGE_N` (12) AND the Wilson-95%-CI **lower bound clears 50%** — a bare-majority fluke is silent. HOLDs are appended to `logRecsToLearningLoop()` after the UI filter strips them (`_holdRecs`, analysis.js); `POST /api/learning/log` dedupes to one HOLD per ticker per 7d (`_HOLD_DEDUP_DAYS`), returning the existing id with `deduped:true`. **HOLD only reaches the Claude pipeline since 2026-07-08** (tool enum re-admit) — no HOLD outcome can resolve before ~2026-08-07 (30-day gate).
...
88. **Paper/real firewall — `isRealTrade(x)` is the ONLY gate.** `x.mode === 'real'`. Missing/null/'paper' → paper. Failing safe: paper can never leak into tax records. Real-only surfaces: EOFY pack, CGT liability summary, NAV history, ATO-facing CSV exports. `adjustCashForMode(delta, mode)` routes to `state.cash` or `state.paperCash`. `askTradeMode()` modal blocks until user chooses — no default. `paperCash` seeded lazily from `settings.paperStartCash`. `splitParcel()` inherits parent mode. `rollbackTradeJournalEntry()` routes cash through mode. `_recomputeHoldingFromParcels()` rebuilds per mode separately. `trade_mode` in `_ALLOWED_OUTCOME_COLS` so mode is upgradeable at execution time. `portfolioViewMode` toggle on Portfolio page (default `'all'`) — **also drives the Performance, Dashboard, Recommendations, Risk, and Backtest (AI Replay tab) pages** (all render the same All/Live/Paper segmented control driving the one global; trade/rec metrics filter by it via `_modeMatch`/`_recModeMatch`; portfolio helpers already follow it). Risk Dashboard is book-scoped through `mergedPortfolio()`/`portfolioValue()`/`_viewModeHoldings()` (incl. rebalance suggestions via `viewCash()`); Backtest's book filter applies ONLY to the AI-Replay tab (`_recModeMatch` on executed recs) — the Technical/Walk-Forward tabs are book-agnostic strategy sims. NAV history + the Performance equity curve are always real-only regardless of toggle (hidden in Paper view, with a note). **Paper trading has NO funds/available-cash concept** — `viewCash()` returns real cash for `all`/`real` and `0` for `paper` (never sums `paperCash`); the Portfolio Cash card is hidden in Paper view; the Dashboard shows no paper cash. **Paper lodging is never gated on funds** — the day-trade cost>cash guards only fire for `isRealTrade(mode)`. `paperCash`/`adjustCashForMode` still track a paper pool internally but it is never surfaced. `GET /api/learning/stats` returns `real_vs_paper` (warns when WR gap ≥15pp n≥5 each side). **Mode filters:** Trade Journal (`journalFilter.mode`), CGT Open Parcels (`_cgtFilter.parcel.mode`), and CGT Disposal History (`_cgtFilter.disposal.mode`) all have an All/Live/Paper `Book` filter (`(x.mode||'paper')` match). The disposal-history filter is **display-only** — the list already shows both books with ●LIVE/◦PAPER badges, and the filter just narrows the view + its footer totals. The tax firewall stays intact where it matters: `exportDisposalCSV`/EOFY pack force `isRealTrade` (real-only) regardless of the Book selection, and the FY summary cards use `realDisposals` only. **Value History** page has a Live/Paper toggle (`_historyMode` in `charts.js`): Live plots real `netWorth/portfolioValue/cash`; Paper plots `paperValue` (paper holdings mkt value, no cash) via `paperPortfolioValue()`. `recordPortfolioSnapshot()` captures `paperValue` per snapshot going forward — historical snapshots have it undefined (chart/table skip null points, `_plottable` gate).
...
91. **Holding `mode` is a persisted `portfolio` column with `UNIQUE(ticker, account, mode)`.** Before this (FIXES.md #1/#2, 2026-07-03), `mode` was dropped on every `db_save` → the real book silently demoted to paper on reload (poisoned the NAV series), and a ticker held real+paper in one account raised an `IntegrityError` that 500'd the whole save (swallowed by `saveStateToDb`). All three of `db_save`, `db_load`, and `_validHolding()` (api.js) must carry `mode` (fail-safe `'paper'`). A ticker can legitimately be TWO portfolio rows (real + paper) — the wider UNIQUE allows it. `saveStateToDb()` now surfaces ≥2 consecutive non-409 save failures (silent persistent save failure is this app's worst failure mode). **Legacy backfill:** `POST /api/portfolio/backfill-modes` (dry-run first; Settings → Data maintenance) is a one-time tool to mark pre-firewall records real — it preserves the explicitly-paper CGT parcels + anything linked by id, bumps `_state_version`, and the client reloads so the migration isn't clobbered. `recordPortfolioSnapshot()` skips (+`console.warn`) a `$0` real PV while open real parcels exist (plausibility guard, FIXES.md #6). **Paper book reset:** `resetPaperBook()` (`utils.js`; Settings → Reset paper book) wipes the sandbox — its delete predicate is `_isExplicitPaper(x)` (`mode==='paper'`), the INVERSE fail-safe of `isRealTrade`: a missing mode is preserved (may be un-backfilled real), so no real/untagged row is ever deleted. **Performance drawdown is real-only:** the drawdown alert banner + Current/Max Drawdown cards derive from real NAV, so they're gated out of Paper view (`_showDd = vm !== 'paper'`), like the equity curve. **Day-trade ML down-weights paper 0.5×:** `trade_snapshots.trade_mode` feeds a `sample_weight` in `_ridge_regression` (paper 0.5, real/NULL 1.0) — uniform weights are identical to unweighted, so it's dormant until paper day-trades are tagged. **Realistic paper fills:** `markExecuted()` slips PAPER fills adversely via `paperFillPrice()` (utils.js) using the shared ADV-tier × regime model (mirror of `core.adv_slippage` + `_SLIP_REGIME_MULT`); gated on `!isRealTrade(mode)` + `settings.paperRealisticFills` (default on). Real trades never slipped. Go-live scorecard passes `friction_bps=0` when the setting is on (P&L already includes friction).

94. **Success-tag taxonomy has two independent sources that must both be styled in `WIN_TAG_META`.** The deterministic auto-tagger (`_classify_success_tags_deterministic`, routes/learning.py) emits 5 tags on every closed win: `regime_aligned`, `disciplined_hold`, `clean_path`, `thesis_confirmed`, `strong_process`. The manual 🔬 skill-score button can additionally apply any of `VALID_WIN_TAGS` (routes/debate.py) — a wider, more qualitative 9-tag set not derivable from captured data alone (`catalyst_capture`, `confluence_entry`, `good_sizing`, `fundamental_value_entry`, `earnings_revision_driven`, `macro_tailwind_aligned`, `contrarian_mean_reversion`, plus the 2 shared tags). `WIN_TAG_META` (js/pages/learning.js) must cover both sets — it previously only had the manual/LLM 9, so the 3 deterministic-only tags fell through to unstyled fallback text on every real occurrence. Two duplicate `WIN_TAG_COLORS` maps (Recent Events chips; one dead/unused near the Lessons card, removed) must independently mirror any new tag added to `WIN_TAG_META`.

94b. **Correlation-drag calibration nudge (`⚠CORR_DRAG`).** `max_corr_to_holdings` is snapshotted into the `market_context` JSON column on every BUY/TOP_UP rec (js/analysis.js, the same correlation check that already halves/cuts new-position size in §9.3) but was dormant in calibration until wired into `_calib_compute()` step 6d (routes/learning.py). Buckets closed BUY/TOP_UP by `|max_corr_to_holdings| > 0.7` (matching analysis.js's own high-correlation cutoff) vs everything else; CI-gated (`_ci_excludes`) on the high-corr bucket's Wilson interval vs the low-corr bucket's raw win rate, both sides needing `_MIN_NUDGE_N` (12). SELL/TRIM/HOLD and virtual (never-executed) rows never enter either bucket — correlation-to-holdings is a new-money concept and virtual rows carry no `market_context`.

94c. **`HOLDING_CONTEXT` now injects a computed entry→now Delta, not just entry-time signals.** `analysis.js` calls `computeThesisDrift()` (js/learning-loop.js) when building the block, reusing its existing `{verdict, reason, deltas}` output (`reason` is already a compact "RSI 42→68" style string) — previously the block only had entry-time `EntrySignals`, so prompts.js's Rule 5 instruction to cite an "entry-vs-now signal delta" had no "now" value to actually compare against and Claude had to reconstruct it. `PROMPT_VERSION` bumped to `2026-07-v22` for the Rule 5 wording change (now says "cite the computed Delta... do not re-derive it yourself").

95. **Multi-device DB sync via git — single-writer discipline + checkpoint-on-shutdown.** The DB is shared across devices by committing `asx_trader.db` (the `-wal`/`-shm` sidecars are git-ignored), so the committed file is only complete if the WAL has been folded in: `db.checkpoint_truncate()` runs `PRAGMA wal_checkpoint(TRUNCATE)` via `atexit` on clean server shutdown. A versioned pre-commit hook (`scripts/hooks/pre-commit`; activate per clone with `git config core.hooksPath scripts/hooks`) refuses to commit `asx_trader.db` while `asx_trader.lock` exists (server running) or `asx_trader.db-wal` is non-empty. Workflow rules: `git pull` BEFORE starting the server; stop the server, then commit+push AFTER; never run servers on two devices concurrently (the single-instance lockfile only protects one machine). A binary-file merge conflict on `asx_trader.db` is unresolvable — pick one side wholesale (`git checkout --theirs/--ours asx_trader.db`), never attempt a merge.

96. **News headline-fallback gate measures content beyond the title (N2b).** Google News RSS entries (Kalkine Media etc.) often set content to literally the title + publisher name — long enough to pass an absolute-length check, but zero real information, and weak local models burn all 3 classify attempts on them (→ `processed=-2`). `_content_beyond_title(title, content)` (news_engine.py) normalises whitespace/case, removes the title from the content, and returns the remaining length; the pipeline's headline-fallback gate compares THAT against `_HEADLINE_FALLBACK_MIN_CONTENT` (40). It strictly subsumes the old absolute check — don't reintroduce a bare `len(content)` gate.

94d. **Rule Override Efficacy card promoted in the Learning page assembly** (`_renderLearningContent`'s return statement) — moved from 28th position (near-bottom, below the Debate Engine's supplementary material) to right after Go-Live Readiness / Coverage, before the Confidence Calibration table. Verdict badges bumped `font-weight:600→700` + a background box-shadow ring; a headline `_decisiveStrip` above the table surfaces `too_strict`/`validated` verdicts as standalone chips so they don't require scanning a table column.

93. **Backtest walk-forward compounds folds; slice equity is index-anchored.** `backtest_walk_forward()` sizes each fold off `oos_capital = capital / n_splits` in isolation, then CHAINS them: the combined OOS curve rebases each fold's per-fold equity onto the running balance and `oos_return_pct = running/capital − 1 = Π(1+fold_ret)−1`. The old code scaled every fold back to `capital` at each boundary, so the headline OOS number reflected only the LAST fold — never reintroduce a per-point `scale = capital/oos_capital` rebase. `_run_strategy_slice()` returns `equity_start_idx` (local index of `equity[0]`, = `max(warmup, trade_from_idx)`); WF maps dates via `burn_start + equity_start_idx + j`, NOT `test_start + j` (they diverge when `warmup > actual_burnin`, e.g. sma_crossover `slow_period=100`). `BURNIN=110` must cover the widest grid warmup. **Opt-in realism (defaults preserve legacy same-bar-close):** `fill_timing='next_open'` defers signal fills to the next bar's `Open` via `pending_buy`/`pending_sell` (ATR stops stay intrabar, gap-through fills at the worse of open/stop); `atr_stop_mult>0` overlays a protective stop (`entry − N×ATR`) that closes with `exitReason:'atr_stop'`. Both validated (`fill_timing∈{close,next_open}`, `atr_stop_mult∈[0,10]`). The main `backtest()` equity curve is built in the SAME pass as trades (`equity_for_ticker`) — the old duplicate second loop was deleted; don't reintroduce it. `backtest_ai_replay()` fetches ONE history per ticker over the widest date window via `fetch_with_retry` (cached in `hist_cache`; an `Exception` sentinel surfaces per-trade), not one fetch per rec.

92. **Failed-quant-rule capture is a stable enum that must stay in sync across FOUR sites.** `_classifyRuleWarnings()` (js/analysis.js) maps a rec's ephemeral `_ruleWarnings` strings + flags (`_confidenceHeld`, `_fallbackSized`, `_budgetBlocked`/`_budgetScaled`) into `{code, detail, severity}` codes: `quant_declined | neg_ev | confidence_floor | whipsaw | self_contradiction | heat_budget | validator_fail | fallback_sized | tier2_rec | factor_thin`. That enum is duplicated in `_RULE_CODE_LABELS` (routes/learning.py); the scrutinize prompt (routes/debate.py) reads `code`/`detail` generically (no per-code list), so new codes flow through it automatically — but **`_RULE_CODE_LABELS` must be updated whenever `_classifyRuleWarnings` gains a code.** `tier2_rec` (a BUY/TOP_UP on a ticker shown only as a compressed Tier-2 one-liner — fabrication risk) and `factor_thin` (fewer than 3 `[FUNDAMENTAL]`/`[MACRO]`-tagged factors on a BUY/TOP_UP) are soft flags added 2026-07 from the Critic B review; both surface via `_ruleWarnings` in analysis.js post-processing, not the validator, so they flag-don't-drop. Persisted to `ai_learning_events.rule_warnings_json` at log time (NULL when clean — the efficacy resolver only scores breached recs). `GET /api/learning/rule-override-efficacy` cross-tabs breached outcomes vs the whole-book baseline: CI-above-baseline → `too_strict` (rule over-cautious), CI-below → `validated` (keep it), gated by `min_n` (default 12) + Wilson CI. **Win-rate + avg P&L use only CLOSED trades** — an executed-but-still-open BUY/TOP_UP realizes no P&L until its parcel is sold, so it lands in a separate `open_n` bucket (the card's drill-down shows its live mark-to-market since entry, computed client-side from `state.liveSignals`). Verdict gates on `closed_n`. Feeds three surfaces: (a) the scrutinize prompt so the local critic weighs the flags; (b) the Learning-page "🧪 Rule Override Efficacy" card (`renderRuleEfficacyCard`); (c) a `RULE-OVERRIDE CALIBRATION` nudge injected into the portfolio prompt (analysis.js, only for decisive verdicts). `_validatorFixed` is deliberately EXCLUDED — auto-repaired ≠ overridden, no learning signal. **Journal mirror (Phase 4):** `markExecuted()` stamps `ruleWarnings`+`overrodeRules` onto the BUY/TOP_UP journal row, persisted via the `trade_journal.rule_warnings_json` column (explicit-column table, no extra_json passthrough — must be wired in `db_save`/`db_load` both), rendered as a "⚠ override" badge + drawer block in journal.js.
