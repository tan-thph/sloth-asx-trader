...
| `js/prompts.js` | All system prompts. `PROMPT_VERSION = '2026-07-v21'`. `scenarios` required non-HOLD. `bullCase` required BUY/TOP_UP. `bearCase` required SELL/TRIM and BUY/TOP_UP ≥0.70 confidence. `invalidationCondition` required non-HOLD. `primary_entry_driver` required BUY/TOP_UP. Response shape is `{recs, summary, dataGaps}` — anti-churn/timing holdbacks go in `dataGaps[]` (no separate `deferrals[]`). HOLD + SELL/TRIM recs with a HOLDING_CONTEXT block must lead with the original EntryDriver's fundamental status and cite an entry-vs-now signal delta (Section 2B + Rule 5). **Book-level "portfolio synthesis" is NOT LLM-emitted** — it is computed deterministically in `analysis.js` (top-sector/top-name concentration + cash %, plus a compact changes-since-last-review tail) and rendered as the "📊 Book snapshot" panel. The verbose `CHANGES SINCE LAST REVIEW` block (engine-computed price/regime/stance diff vs the prior run's persisted `state.analysisLastSummary.review` snapshot — `{regime, prices, recs}`) is still injected into the prompt so recs focus on new information. |
