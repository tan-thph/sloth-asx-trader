...
| `core.py` | ~230 | Shared infrastructure: `ttl_cache`, `fetch_with_retry` (exponential backoff + `_last_good` stale-cache), `stooq_quote(yf_symbol)` + `_to_stooq_sym()` (Stooq price fallback — third line of defence when `_last_good` is also empty; covers all `.AX` equities + major global symbols; `TIO=F`/`YAP=F` unsupported), `_HTTP_SESSION`, `log`, `LOG_DIR`, `SECTOR_MAP`, `ASX_UNIVERSE`, `OLLAMA_BASE`. No Flask import. |
