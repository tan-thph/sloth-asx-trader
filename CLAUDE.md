...
- `GET /api/learning/rule-override-efficacy` — per failed-rule-code WR/avg-P&L of breached-AND-executed trades vs baseline; verdict `too_strict`/`validated`/`inconclusive` (Wilson-CI + `min_n` gated)
...
92. **Failed-quant-rule capture is a stable enum that must stay in sync across FOUR sites.** `_classifyRuleWarnings()` (js/analysis.js) maps a rec's ephemeral `_ruleWarnings` strings + flags (`_confidenceHeld`, `_fallbackSized`, `_budgetBlocked`/`_budgetScaled`) into `{code, detail, severity}` codes: `quant_declined | neg_ev | confidence_floor | whipsaw | self_contradiction | heat_budget | validator_fail | fallback_sized`. That enum is duplicated in `_RULE_CODE_LABELS` (routes/learning.py) and read by the scrutinize prompt (routes/debate.py) — **change one, change all.** Persisted to `ai_learning_events.rule_warnings_json` at log time (NULL when clean — the efficacy resolver only scores breached recs). `GET /api/learning/rule-override-efficacy` cross-tabs breached-AND-executed outcomes vs the whole-book baseline: CI-above-baseline → `too_strict` (rule over-cautious), CI-below → `validated` (keep it), gated by `min_n` (default 12) + Wilson CI. Feeds three surfaces: (a) the scrutinize prompt so the local critic weighs the flags; (b) the Learning-page "🧪 Rule Override Efficacy" card (`renderRuleEfficacyCard`); (c) a `RULE-OVERRIDE CALIBRATION` nudge injected into the portfolio prompt (analysis.js, only for decisive verdicts). `_validatorFixed` is deliberately EXCLUDED — auto-repaired ≠ overridden, no learning signal. **Journal mirror (Phase 4):** `markExecuted()` stamps `ruleWarnings`+`overrodeRules` onto the BUY/TOP_UP journal row, persisted via the `trade_journal.rule_warnings_json` column (explicit-column table, no extra_json passthrough — must be wired in `db_save`/`db_load` both), rendered as a "⚠ override" badge + drawer block in journal.js.
