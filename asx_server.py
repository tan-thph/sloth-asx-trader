...
if __name__ == "__main__":
    ...
    import subprocess as _sp, threading as _th, atexit as _atexit
    _ngrok_proc = None
    ...
    def _start_ngrok():
        global _ngrok_proc
        try:
            _ngrok_proc = _sp.Popen(
                ["ngrok", "http", "5000"],
                stdout=_sp.PIPE, stderr=_sp.STDOUT,
            )
            # Wait briefly then read the public URL from the ngrok local API.
            # ngrok falls back to ports 4041-4049 when 4040 is already in use,
            # so probe all of them rather than assuming 4040.
            import time as _time, urllib.request as _ur, json as _json
            _time.sleep(3)
            url = None
            for _port in range(4040, 4050):
                try:
                    resp = _ur.urlopen(f"http://127.0.0.1:{_port}/api/tunnels", timeout=2)
            ...
            if url:
                ...
                # Surface the ngrok stderr so the user knows why it failed
                try:
                    out = _ngrok_proc.stdout.read(2000).decode(errors="replace").strip()
                    if out:
