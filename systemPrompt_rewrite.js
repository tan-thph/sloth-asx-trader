...
ADDITIONAL FACTORS (assess where data permits):
  ...
  FACTOR CLASSIFICATION: Label each holding as one or more of:
    ...
    Flag factor crowding if ≥ 40% of portfolio in the same factor ("exceeds threshold");
    warn "approaching threshold" if 35–39.9%.
...
PRE-FLIGHT VERIFICATION (run before returning JSON — fix any failure before output):
  ...
  (f) Every BUY/TOP_UP has a non-empty bearCase regardless of confidence level.
...
For each holding provide:
  ...
  scenarios: { bull: {p, ret}, base: {p, ret}, bear: {p, ret} }
    ...
    — scenario returns must be internally consistent with the stated target and stopLoss:
        bull ret  ≈ (target / limitPrice − 1) — the target being hit
        base ret  ≈ (target / limitPrice − 1) × 0.55 — partial move toward target
        bear ret  ≈ −(limitPrice / stopLoss − 1) — stop loss being hit
      A bull scenario significantly exceeding (target / limitPrice − 1) is a red flag and
      should be corrected before output.
  ...
  grossedUpYield: number (REQUIRED if the stock pays a dividend — rawYield × (1 + frankingPct × 0.30/0.70);
                  null only if no dividend is paid. Apply consistently across all recs for that ticker)
...
sectorBreakdown:         Each sector as % of portfolio; flag any > 30%.
                         IMPORTANT: each holding must appear under exactly ONE sector key.
                         Never create a separate key for an individual stock that is already
                         counted inside a broader sector entry — doing so double-counts it.
...
factorCrowding:          Flag as "exceeds threshold" if ≥ 40% in same factor.
                         Flag as "approaching threshold" if 35–39.9%. Include diversification
                         suggestion in both cases. Never say "exceeds" when the value is below 40%.
...
betterOpportunities:     3–5 ASX stocks/ETFs not currently held, better risk-adjusted potential.
                         ...
                         EXCLUDE any name where RSI > 75 and the current price is already above the
                         preferred entry level — do not list overbought names as "opportunities".
                         If a name has potential but is overbought, note it as "watch for pullback
                         to [price]" rather than listing it as a current buy opportunity.
...
Each rec object — ALL fields required unless marked optional:
{
  ...
  "priceRange":            [low, high],
  "target":                number,
  "stopLoss":              number (for BUY/TOP_UP: price below which thesis is invalid — exit position;
                             for TRIM/SELL: minimum acceptable execution price — abandon the trim and
                             hold if price drops below this level before the order fills),
  "qty":                   number,
  ...
  "bearCase":              string (MAX 100 chars; REQUIRED for ALL BUY/TOP_UP recs —
                             one-sentence steelman of how this trade loses money in 90 days;
                             if no credible bear case exists, downgrade confidence by 0.10;
                             omit only for SELL/TRIM actions),
  "weightGuidance":        "Strong Accumulate (+3–5%)" | "Accumulate (+1–2%)" | "Hold" | "Reduce (−25–50%)" | "Exit",
  "expectedProfit":        number (gross dollar profit:
                             BUY/TOP_UP → (target − limitPrice) × qty;
                             TRIM/SELL  → (limitPrice − avgCostBasis) × qty — state the cost basis used),
  "brokerageCost":         ${state.settings.brokerage},
  "apiCost":               ${state.settings.apiCostPerRun},
  "netProfit":             number — MUST apply this formula consistently across ALL recs:
                             netProfit = expectedProfit − brokerageCost − apiCost
                             (brokerageCost and apiCost are always the values above; never halve or
                             omit either for any rec type — consistency is required),
  "taxBenefitEstimate":    number (optional — tax-loss harvest recs only),
  "grossedUpYield":        number (REQUIRED for any ticker that pays a dividend — compute as
                             rawYield × (1 + frankingPct × 0.30/0.70); set null only if the stock
                             pays no dividend at all. Must be present consistently across ALL recs
                             for dividend-paying holdings — not just income-themed recs)
}
