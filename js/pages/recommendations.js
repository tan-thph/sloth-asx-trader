...
function renderRecommendations() {
  ...
  return `
    <div class="tabs">
      ...
      <button class="tab" id="tab-run" onclick="switchRecTab('run')">â–¶ Run Analysis</button>
      ...
      <button class="tab" id="tab-rules" onclick="switchRecTab('rules')">âš™ Rules</button>
    </div>
  ...
  return `
    <div style="display:flex;flex-direction:column;gap:14px">
        ...
        <div class="flex-row" style="flex-wrap:wrap;gap:6px;margin-bottom:10px" id="extra-ticker-chips">
          ${extraTickers.map(t=>`
            <span style="display:inline-flex;align-items:center;gap:5px;padding:4px 10px;background:#dbeafe;color:#1d4ed8;border-radius:6px;font-size:12px;font-weight:600;border:0.5px solid #93c5fd">
              ${t}
              <button onclick="removeExtraTicker('${t}')" style="background:none;border:none;cursor:pointer;color:#1d4ed8;font-size:13px;padding:0;line-height:1">Ã—</button>
            </span>`).join('')}
        </div>
      ...
      <div class="card">
        ...
        <div class="form-row">
          <textarea id="market-view-input" rows="5" placeholder="e.g. RBA likely to cut rates in Q3 â€” favour rate-sensitive sectors (REITs, banks). Iron ore softening â€” cautious on BHP/FMG near-term. Looking for yield plays above 4%. Avoid adding more mining exposure. Expecting volatility ahead of US CPI this week."
            ...
            ${cfg.savedViews.slice().reverse().map((v,ri)=>{
              ...
              <div style="display:flex;align-items:flex-start;gap:8px;padding:8px 10px;background:var(--bg-secondary);border-radius:var(--radius-md);border:0.5px solid var(--border-light)">
                ...
                <div class="flex-row" style="gap:4px;flex-shrink:0">
                  <button class="btn btn-sm" onclick="restoreSavedView(${i})" title="Restore this view">â†© Use</button>
                  <button class="btn btn-sm btn-danger" onclick="deleteSavedView(${i})" title="Delete">âœ•</button>
                </div>
              </div>`;
            }).join('')}
          </div>
        ...
        <div class="flex-row" style="margin-top:10px;gap:8px">
          <button class="btn btn-sm" onclick="saveCurrentView()">ðŸ’¾ Save view</button>
      ...
      <div class="card">
        ...
        <div style="background:var(--bg-secondary);border-radius:var(--radius-md);padding:12px 14px;margin-bottom:14px;font-size:13px;line-height:1.7">
          <div><strong>Tickers to analyse:</strong> <span style="font-weight:600;color:var(--text-primary)">${allTickers.join(', ') || 'â€”'}</span>
          ...
          <div><strong>Market view:</strong> ${cfg.marketView ? `<span style="color:var(--text-primary)">${cfg.marketView.substring(0,80)}${cfg.marketView.length>80?'â€¦':''}</span>` : '<span class="text-muted">None (general analysis)</span>'}</div>
          ${!state.serverOk?`<div style="color:#d97706;margin-top:4px">âš  Backend offline â€” analysis will use cached/no live data</div>`:''}
        </div>
            ...
            ${(()=>{
              ...
              return `<div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
                <select id="analysis-model-select" onchange="_onAnalysisModelSelect(this.value)" style="${_selStyle}">
                  ...
                  <option value="__custom__" ${_isCustom?'selected':''}>Customâ€¦</option>
                </select>
        ...
        <div class="flex-row">
          <button class="btn btn-primary" onclick="runAnalysisFromPanel()" ${state.analysisRunning?'disabled':''} style="${state.analysisRunning?'opacity:0.6':''}" id="run-analysis-btn">
            ${state.analysisRunning ? '<span class="loading-dots"><span></span><span></span><span></span></span> Running...' : 'â–¶ Run Full Analysis'}
          </button>
          <span class="text-xs text-muted">Analyses all ${allTickers.length} tickers Â· ~$${(state.settings.apiCostPerRun||0.08).toFixed(2)} API cost</span>
          ${(()=>{
            const td = todayStr();
...
function _sectorConcentrationWarning(rec) {
  ...
  return `<span style="background:${bg};color:${col};border-radius:3px;padding:1px 6px;font-size:10px;font-weight:600;white-space:nowrap" title="Sector ${rec.sector} is ${sectorWt.toFixed(0)}% of portfolio (max ${maxSect}%) via ${names}">âš  ${rec.sector} ${sectorWt.toFixed(0)}%</span>`;
}

// Renders the Analyst Summary block â€” structured card rows when `summary.recs` is
...
function _renderSummaryBlock(summary, opts = {}) {
  ...
  const header = `<div style="font-size:11px;font-weight:600;color:var(--text-tertiary);text-transform:uppercase;letter-spacing:0.6px;margin-bottom:10px">
    Analyst Summary Â· ${escapeHTML(summary.time || '')} ${escapeHTML(summary.date || '')}
  </div>`;
    ...
    const rows = summary.recs.map((r, i) => {
      ...
      return `<div style="display:flex;gap:8px;align-items:baseline;padding:6px 0${isLast ? '' : ';border-bottom:1px solid var(--border)'}">
        ...
        <div style="min-width:0;line-height:1.55;font-size:13px">
          ...
          ${r.reasoning ? `<span style="color:var(--text-secondary)"> â€” ${escapeHTML(r.reasoning)}</span>` : ''}
        </div>
      </div>`;
    }).join('');
  ...
  return `<div style="background:${bgColor};border-radius:var(--radius-md);padding:14px 16px;border-left:3px solid ${borderColor}">${header}${body}</div>`;
}

// â”€â”€ Shared rec-card body renderer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// When isHistory=false  â†’ wraps in <div class="rec-card"> with execute buttons
// When isHistory=true   â†’ returns body-only HTML for injection into the detail
...
function _buildRecCardHTML(r, opts) {
  ...
  // Account resolution (pending mode only â€” history recs have no live holding context)
  ...
  // Entry status tag (pending only â€” shows if live price is inside/outside entry range)
  let entryStatusTag = '';
  if (!isHistory && !isReducing && livePx != null && r.priceRange?.[0] != null && r.priceRange?.[1] != null) {
    const [lo, hi] = r.priceRange;
    if (livePx < lo)
      entryStatusTag = `<span style="font-size:10px;padding:1px 5px;background:#dcfce7;color:#15803d;border-radius:3px">â–¼ ${fmt(((lo - livePx) / lo) * 100, 1)}% below entry</span>`;
    else if (livePx > hi)
      entryStatusTag = `<span style="font-size:10px;padding:1px 5px;background:#fee2e2;color:#dc2626;border-radius:3px">â–² ${fmt(((livePx - hi) / hi) * 100, 1)}% above entry</span>`;
    else
      entryStatusTag = `<span style="font-size:10px;padding:1px 5px;background:#fef3c7;color:#d97706;border-radius:3px">â— In entry range</span>`;
  } else if (!isHistory && isReducing && livePx != null && sellPrice != null) {
    const dist = ((sellPrice - livePx) / livePx) * 100;
    entryStatusTag = dist >= 0
      ? `<span style="font-size:10px;padding:1px 5px;background:#dcfce7;color:#15803d;border-radius:3px">â–¼ ${fmt(dist, 1)}% to sell target</span>`
      : `<span style="font-size:10px;padding:1px 5px;background:#fee2e2;color:#dc2626;border-radius:3px">â–² ${fmt(Math.abs(dist), 1)}% above target</span>`;
  }
  ...
  const outcomeBadge = isHistory ? (
    isSkipped
      ...
      : r.outcome === 'win'
        ? `<span class="badge" style="background:#f0fdf4;color:#15803d;border:1px solid #86efac">âœ“ Won</span>`
        : r.outcome === 'loss'
          ? `<span class="badge" style="background:#fef2f2;color:#dc2626;border:1px solid #fca5a5">âœ— Lost</span>`
          : `<span class="badge" style="background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe">â— Open</span>`
  ) : '';
  ...
  const costTileDetail = parcelCost != null
    ? `${r._parcelLabel ? r._parcelLabel + ' Â· ' : ''}cost = $${fmt(parcelCost * qty)}${r._parcelRemainingQty && r._parcelRemainingQty !== qty ? ` (lot has ${r._parcelRemainingQty} shares)` : ''}`
    : 'â€”';

  const topTiles = isHistory
    ? (isReducing
        ? `${tile('Sell at (AI target)', `<span class="text-success">$${fmt(sellPrice)}</span>`, `${qty} share${qty===1?'':'s'} Â· stop $${fmt(r.stopLoss)}`)}
           ${tile('Target / Stop', `<span class="text-success">$${fmt(r.target)}</span> / <span class="text-danger">$${fmt(r.stopLoss)}</span>`)}
           ${tile('Actual P&L', histPnlVal != null ? `<span style="color:${histPnlColor}">${histPnlSign}$${fmt(Math.abs(histPnlVal))}</span>` : '<span class="text-muted">â€”</span>', histPnlVal != null ? 'Recorded at close' : 'Pending')}`
        : `${tile('Entry range (AI)', r.priceRange ? `$${fmt(r.priceRange[0])} &ndash; $${fmt(r.priceRange[1])}` : 'â€”')}
           ${tile('Target / Stop-loss', `<span class="text-success">$${fmt(r.target)}</span> / <span class="text-danger">$${fmt(r.stopLoss)}</span>`)}
           ${tile('Actual P&L', histPnlVal != null ? `<span style="color:${histPnlColor}">${histPnlSign}$${fmt(Math.abs(histPnlVal))}</span>` : '<span class="text-muted">â€”</span>', histPnlVal != null ? 'Recorded at close' : isSkipped ? 'Skipped' : 'Position open')}`)
    : (isReducing
        ? `${tile('Sell at (limit)', `<span class="text-success">$${fmt(sellPrice)}</span>`, `${qty} share${qty===1?'':'s'} Â· stop if breaks $${fmt(r.stopLoss)}`)}
           ${tile(costTileLabel, parcelCost != null ? `$${fmt(parcelCost)} <span class="text-muted">Ã— ${qty}</span>` : '<span class="text-muted">no holding</span>', costTileDetail)}
           ${tile('Net realised P&L (est.)', `<span style="color:${pnlColor}">${displayPnl!=null?pnlSign+'$'+fmt(Math.abs(displayPnl)):'<span style="color:var(--text-tertiary)">â€”</span>'}</span>`, parcelCost != null ? `($${fmt(sellPrice)} âˆ’ $${fmt(parcelCost)}) Ã— ${qty} âˆ’ $${fees} fee` : `AI estimate Â· cost basis unavailable`)}`
        : `${tile('Entry range', `$${fmt(r.priceRange[0])} &ndash; $${fmt(r.priceRange[1])}`)}
           ${tile('Target / Stop-loss', `<span class="text-success">$${fmt(r.target)}</span> / <span class="text-danger">$${fmt(r.stopLoss)}</span>`)}
           ${tile('Est. P&L if target hit', `<span style="color:${pnlColor}">${displayPnl!=null?pnlSign+'$'+fmt(Math.abs(displayPnl)):'<span style="color:var(--text-tertiary)">â€”</span>'}</span>`, displayPnl!=null?`(target âˆ’ entry) Ã— ${qty} âˆ’ $${fees*2} round-trip`:'No target/entry data')}`);

  const qtyLine = r.action === 'TRIM'
    ? (r._parcelRemainingQty ? `Trim: ${qty||'â€”'} of ${r._parcelRemainingQty} shares (${r._parcelLabel||'parcel'})` : `Trim: ${qty||'â€”'} of ${isHistory ? qty : heldQty} shares`)
    : r.action === 'TOP_UP' ? `Add: ${qty||'â€”'} shares`
    : r.action === 'SELL'
    ? (r._parcelLabel ? `Sell: ${qty||'â€”'} shares (${r._parcelLabel})` : `Sell: ${qty||'â€”'} of ${isHistory ? qty : heldQty} shares`)
    : `Qty: ${qty||'â€”'} shares`;
  ...
  const headerHTML = isHistory ? `
    <div class="flex-between mb-1">
      <div class="flex-row" style="flex-wrap:wrap;gap:5px">
        ...
        ${r._confidenceHeld ? '<span class="badge" style="background:#fef2f2;color:#dc2626;border:1px solid #fca5a5;font-weight:700" title="Confidence was below the floor at generation">â›” HELD</span>' : ''}
        ...
        ${regimeBadge}
        ${r._source==='local' ? '<span class="badge" style="background:#fff7ed;color:#92400e;border:none">ðŸ”’ Local</span>' : ''}
        ${r._stopWidened ? `<span class="badge" style="background:#fff7ed;color:#c2410c;border:1px solid #fed7aa" title="Stop widened: $${(r._stopWidenedFrom??0).toFixed(2)} â†’ $${(r.stopLoss??0).toFixed(2)}">â†” Widened</span>` : ''}
        ${outcomeBadge}
        ${actualPnlBadge}
        <span class="text-xs text-muted">${escapeHTML(r.date)}</span>
      </div>
      <div class="flex-row">
        <button class="btn btn-sm" onclick="showRecTraceability('${r.id}')" title="Why this rec? â€” full decision trail">â„¹ï¸</button>
      </div>
    </div>` : `
    <div class="flex-between mb-1">
      <div class="flex-row">${actionBadge(r.action)}${r._confidenceHeld ? '<span class="badge" style="background:#fef2f2;color:#dc2626;border:1px solid #fca5a5;font-weight:700" title="AI confidence is below the minimum threshold â€” review carefully before executing">â›” HELD</span>' : ''}<strong style="font-size:15px">${r.ticker}</strong>${r._parcelLabel ? `<span style="font-size:11px;padding:2px 7px;background:#eff6ff;border:1px solid #bfdbfe;border-radius:3px;color:#1d4ed8;font-weight:600" title="CGT parcel ${r._parcelLabel}${r._holdingDays != null ? ' â€” ' + r._holdingDays + ' days held' : ''}">${escapeHTML(r._parcelLabel)}</span>` : ''}${r._splitTotal > 1 ? `<span style="font-size:10px;padding:2px 6px;background:var(--bg-secondary);border:1px solid var(--border-light);border-radius:3px;color:var(--text-muted)">Lot ${(r._splitIndex || 0) + 1}/${r._splitTotal}</span>` : ''}${livePxBadge}${entryStatusTag}<span class="text-xs">${r.sector||''}</span>${_sectorConcentrationWarning(r)}${r._corrNote?`<span class="badge" style="background:#fef3c7;color:#92400e;border:none" title="${escapeHTML(r._corrNote)}">âš¡ Corr</span>`:''}${r._budgetScaled?`<span class="badge" style="background:#fff7ed;color:#c2410c;border:none" title="${escapeHTML(r._budgetNote||'')}">âš  Budget</span>`:''}${r._preEarningsAdj?`<span class="badge" style="background:#eff6ff;color:#1d4ed8;border:none" title="Stop distance widened 30% â€” earnings within 14 days">ðŸ“… Pre-earnings</span>`:''}${r._varAdjusted?`<span class="badge" style="background:#fef2f2;color:#b91c1c;border:none" title="Position size reduced ${r._varMult===0.5?'50%':'25%'} â€” VaR1d ${r._varMult===0.5?'< -5.0%':'< -3.5%'} (high-risk ticker)">ðŸ“‰ VaR adj</span>`:''}${r.isWatchlist?'<span class="badge" style="background:#ede9fe;color:#6d28d9;border:none">â˜… Watchlist</span>':''}${r._source==='local'?'<span class="badge" style="background:#fff7ed;color:#92400e;border:none" title="Generated by local Ollama model â€” simplified 5-driver taxonomy">ðŸ”’ Local</span>':''}${r._stopTrailed?`<span class="badge" style="background:#f0fdf4;color:#15803d;border:1px solid #86efac" title="Stop trailed at ${r._stopTrailedAt||''}">ðŸ“ Trailed</span>`:''}<span class="badge badge-pending">Pending</span><span class="text-xs text-muted">Generated ${r.generatedAt||r.date}</span></div>
      <div class="flex-row">
        <button class="btn btn-sm btn-success" onclick="confirmExecute('${r.id}')">âœ“ Mark Executed</button>
        <button class="btn btn-sm btn-danger" onclick="markSkipped('${r.id}')">âœ• Skip</button>
        <button class="btn btn-sm" onclick="trailStop('${r.id}')" title="Trail stop to current price Ã— ATR">ðŸ“ Trail</button>
        <button class="btn btn-sm" onclick="showRecTraceability('${r.id}')" title="Why did I get this rec? â€” full decision trail">â„¹ï¸</button>
      </div>
    </div>`;

  // â”€â”€ Failed-rule review panel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const failedRulesHTML = _warns.length ? `
    <div style="margin-top:8px;padding:8px 12px;border-radius:var(--radius-md);background:var(--down-bg, rgba(220,38,38,0.08));border:1px solid rgba(220,38,38,0.35);border-left:3px solid #dc2626">
      <div style="font-size:11px;font-weight:700;color:#dc2626;text-transform:uppercase;letter-spacing:0.5px">
        âš  Failed ${_warns.length} rule${_warns.length !== 1 ? 's' : ''}${isHistory ? ' at generation time' : ' â€” would have been dropped. Skip, or execute on your own judgment.'}
      </div>
  ...
  // â”€â”€ Parcel metadata row (SELL/TRIM, when parcel data present) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const parcelMetaHTML = (isReducing && r._parcelLabel) ? (() => {
    const cgtBadge = r._cgtEligible
      ? '<span style="font-size:11px;padding:1px 7px;background:#dcfce7;border:1px solid #86efac;border-radius:3px;color:#15803d;font-weight:600">âœ“ CGT 50% eligible</span>'
      : (r._holdingDays != null && r._holdingDays < 365
          ? `<span style="font-size:11px;padding:1px 7px;background:#fee2e2;border:1px solid #fca5a5;border-radius:3px;color:#dc2626;font-weight:600">âœ— CGT not yet (${365 - r._holdingDays}d to go)</span>`
    ...
    const heldStr   = r._holdingDays != null ? `${r._holdingDays} days` : 'â€”';
    ...
    <div style="padding:6px 10px;border-radius:var(--radius-md);background:var(--bg-secondary);border:1px solid var(--border-light);margin-bottom:8px;font-size:11.5px;display:flex;flex-wrap:wrap;gap:8px;align-items:center">
      ...
      <span style="color:var(--text-secondary)">Entered <strong>${escapeHTML(r._parcelDate || 'â€”')}</strong> Â· <strong>${escapeHTML(heldStr)} held</strong></span>
      <span style="color:var(--border-medium)">Â·</span>
      ${cgtBadge}
      ${unrealStr ? `<span style="color:var(--border-medium)">Â·</span><span style="font-size:11.5px;color:var(--text-secondary)">${unrealStr}</span>` : ''}
    </div>`;
  })() : '';

  // â”€â”€ SELL/TRIM decision tags â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const sellReasonHTML = (isReducing && r.primary_driver) ? (() => {
    ...
    const altBadge = r.alternativeTicker
      ? `<span style="font-size:10px;padding:2px 7px;background:#eff6ff;border:1px solid #bfdbfe;border-radius:3px;color:#1d4ed8" title="Redeploy into ${r.alternativeTicker}">â†’ ${r.alternativeTicker}</span>`
      : '';
    ...
    const reallocLine = r.reallocationSuggestion
      ? `<div style="margin-top:6px;padding:5px 8px;background:var(--bg-inset,var(--bg-secondary));border-radius:4px;border-left:2px solid #60a5fa;font-size:11px;color:var(--text-secondary);display:flex;align-items:flex-start;gap:5px">
          <span style="font-weight:700;color:#3b82f6;white-space:nowrap;flex-shrink:0">â†ª Capital:</span>
  ...
  // â”€â”€ Thesis check (SELL/TRIM) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const thesisCheckHTML = (isReducing && r._thesisCheck) ? (() => {
    const tc = r._thesisCheck;
    const VERDICT_CONFIG = {
      validated:   { icon: 'âœ…', label: 'Validated',   color: 'var(--up)',           bg: 'var(--up-bg, #f0fdf4)',   border: '#86efac' },
      invalidated: { icon: 'âŒ', label: 'Invalidated', color: 'var(--down)',          bg: 'var(--down-bg, #fef2f2)', border: '#fca5a5' },
      irrelevant:  { icon: 'âž–', label: 'Irrelevant',  color: 'var(--text-tertiary)', bg: 'var(--bg-inset)',         border: 'var(--border)' },
    };
    ...
    const fmt2 = v => v != null ? Number(v).toFixed(2) : 'â€”';
    return `
    <div style="padding:8px 12px;background:${vc.bg};border:1px solid ${vc.border};border-left:3px solid ${vc.color};border-radius:var(--radius-md);margin-bottom:8px;font-size:12px">
      ...
      <div style="display:flex;gap:14px;flex-wrap:wrap;margin-bottom:3px">
        <span style="color:var(--text-secondary)">Bought because: <strong>${escapeHTML(r._entryDriver || 'untagged')}</strong>${firstKey ? ` Â· Entry: <strong>${escapeHTML(firstKey.replace(/_/g, ' '))+': '+fmt2(firstDelta?.entry)}</strong>` : ''}</span>
  ...
  // â”€â”€ Scenarios grid â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const scenariosHTML = (r.scenarios?.bull && r.scenarios?.base && r.scenarios?.bear) ? (() => {
    const sc = r.scenarios;
    const fmtPct = v => v != null ? (v >= 0 ? '+' : '') + (v * 100).toFixed(0) + '%' : 'â€”';
  ...
  // â”€â”€ Generation-context traceability panel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const traceHTML = (() => {
    ...
    if (r._signalsSnap) {
      ...
      const cells = [
        ...
        s.volume_ratio != null ? `VolÃ—${fmt(s.volume_ratio,1)}` : null,
    ...
    if (!isHistory && typeof _getCachedDebate === 'function') {
      ...
      if (d && d.bull && d.bear) {
        ...
        const snippet = synth + (d.synthesis?.key_pivot || (d.bull.slice(0,80) + 'â€¦'));
    ...
    <details style="margin-top:8px;border-top:0.5px solid var(--border-light);padding-top:6px">
      <summary style="font-size:11px;color:var(--text-muted);cursor:pointer;user-select:none;list-style:none;display:flex;align-items:center;gap:5px">
        <span style="transition:transform 0.15s" class="trace-arrow">â–¶</span> ðŸ” Why this rec? (generation context)
      </summary>
  ...
  // â”€â”€ Execution form (pending only) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const execFormHTML = isHistory ? '' : `
    <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;padding:10px 12px;background:var(--bg-secondary);border-radius:var(--radius-md);border:1px solid #3b82f6;margin-bottom:10px">
      ...
      ${_rAccts.length > 1 ? `
      <div style="display:flex;align-items:center;gap:6px">
        <label style="font-size:12px;color:#f59e0b;font-weight:600;white-space:nowrap">âš  Account</label>
        <select id="exec-acct-${r.id}"
          ...
          title="${r.ticker} is held in multiple accounts â€” pick which one this ${r.action} applies to">
  ...
  // â”€â”€ Staleness check banner (pending only, â‰¥2 days old) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const stalenessHTML = (isHistory || _recAgeInDays(r) < 2) ? '' :
    `<div id="staleness-${r.id}" style="margin-bottom:8px">
      <div style="display:flex;align-items:center;gap:6px;padding:6px 10px;
        ...
        <span>â³</span>
        <span>Rec is ${_recAgeInDays(r)} day(s) old â€” checking if setup is still validâ€¦</span>
      </div>
    </div>`;

  // â”€â”€ Position sizer (pending BUY/TOP_UP only) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const posSizerHTML = (!isHistory && !isReducing) ? renderPositionSizer(r) : '';

  // â”€â”€ Tags / thesis: inputs for pending, read-only for history â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const thesisHTML = isHistory
    ...
    : `<div style="display:flex;gap:8px;align-items:flex-start;padding-top:6px">
         ...
         <textarea id="thesis-${r.id}" rows="2" placeholder="Write your investment thesis â€” what needs to stay true for this trade to work?"
           ...
           title="Investment thesis â€” reviewed at trade exit">${escapeHTML(r._thesis||'')}</textarea>
       </div>`;
  ...
  const feedbackHTML = isHistory
    ? `<div style="display:flex;gap:8px;align-items:center;padding-top:8px;border-top:0.5px solid var(--border-light)">
         <input type="text" id="hfb-${historyRealIdx}" value="${escapeHTML(r.feedback||'')}" placeholder="Add feedbackâ€¦"
         ...
         <button class="btn btn-sm" onclick="addHistoryFeedback(${historyRealIdx})" title="Save">ðŸ’¬ Save</button>
       </div>
       ${r.feedback ? `<div class="text-xs text-muted" style="margin-top:4px">ðŸ’¬ ${escapeHTML(r.feedback)}</div>` : ''}`
    : `<div style="display:flex;gap:8px;align-items:center;padding-top:8px;border-top:0.5px solid var(--border-light)">
         <input type="text" id="feedback-${r.id}" placeholder="Optional feedback for next analysis run (e.g. 'too aggressive', 'ignore WDS for now')" value="${r.feedback||''}"
           style="flex:1;padding:6px 10px;border-radius:var(--radius-md);border:0.5px solid var(--border-medium);background:var(--bg-secondary);color:var(--text-primary);font-size:12px;font-family:var(--font)">
         <button class="btn btn-sm" onclick="submitFeedback('${r.id}')">ðŸ’¬ Save Feedback</button>
       </div>`;
  ...
  const bodyHTML = `
    ...
    <details style="margin-bottom:8px;background:var(--bg-secondary);border-radius:var(--radius-md);border:0.5px solid var(--border-light)" open>
      <summary style="padding:7px 12px;font-size:10px;font-weight:700;color:var(--text-muted);cursor:pointer;user-select:none;text-transform:uppercase;letter-spacing:0.5px;list-style:none;display:flex;align-items:center;gap:5px">
        <span class="trace-arrow">â–¼</span> Analysis factors (${r.factorsUsed.length})
      </summary>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:8px">
      ${r.bullCase ? `<div style="padding:6px 9px;background:var(--up-bg,rgba(22,163,74,0.08));border:1px solid rgba(22,163,74,0.3);border-radius:var(--radius-md);font-size:11.5px;color:var(--text-secondary)">
        <span style="font-weight:700;color:var(--up,#16a34a);display:block;margin-bottom:2px;font-size:10px;text-transform:uppercase;letter-spacing:0.4px">â–² Bull case</span>
      ...
      ${r.bearCase ? `<div style="padding:6px 9px;background:var(--down-bg,rgba(220,38,38,0.06));border:1px solid rgba(220,38,38,0.25);border-radius:var(--radius-md);font-size:11.5px;color:var(--text-secondary)">
        <span style="font-weight:700;color:var(--down,#dc2626);display:block;margin-bottom:2px;font-size:10px;text-transform:uppercase;letter-spacing:0.4px">â–¼ Bear case</span>
    ...
    <div style="display:flex;align-items:flex-start;gap:7px;padding:6px 10px;background:rgba(217,119,6,0.07);border:1px solid rgba(217,119,6,0.35);border-radius:var(--radius-md);margin-bottom:8px;font-size:11.5px;color:var(--text-secondary)">
      <span style="font-weight:700;color:#d97706;white-space:nowrap;flex-shrink:0;font-size:10px;text-transform:uppercase;letter-spacing:0.4px;padding-top:1px">âš¡ Invalidated if</span>
...
function renderPendingRecs(recs) {
  ...
  if(!recs.length) {
    ...
    return `<div class="card"><div class="empty-state" style="text-align:left;padding:1.5rem">
      <div style="text-align:center;margin-bottom:1rem"><div class="empty-icon">â—†</div></div>
      ${hasRunBefore ? `
        <div style="margin-bottom:1rem">${_renderSummaryBlock(summary)}</div>
        <p class="sub" style="text-align:center">No actionable setups found â€” see reasoning above. Run again when conditions change.</p>
      ` : `
        <p style="color:var(--text-secondary);font-size:14px;text-align:center">No pending recommendations.</p>
        <p class="sub" style="text-align:center">Go to <strong>â–¶ Run Analysis</strong> tab to configure and run. The AI will use live yfinance indicator data when available.</p>
      `}
      <div style="text-align:center;margin-top:1rem">
        <button class="btn btn-primary" onclick="switchRecTab('run')">â–¶ Run Analysis</button>
      </div>
    </div></div>`;
  }
...
// â”€â”€ Entry Staleness Check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
 ...
 * for any pending recs that are â‰¥ 2 days old.  Each check updates its banner div
...
async function initRecStalenessChecks() {
  ...
  if (!status?.available) {
    // Still update the banners to show "Ollama offline â€” verify manually"
    pending.forEach(r => {
  ...
  // Run sequentially â€” one Ollama call at a time avoids memory pressure
...
function _stalenessBanner(recId, result, daysAgo) {
  if (!result?.ok) {
    return `<div style="padding:6px 10px;background:#f8fafc;border:1px solid var(--border);
      ...
      âš  Rec is ${daysAgo} day(s) old â€” Ollama offline, verify setup manually before executing.
    </div>`;
  }
  const cfg = {
    VALID:       { bg: '#f0fdf4', border: '#bbf7d0', color: '#15803d', icon: 'âœ“' },
    WEAKENED:    { bg: '#fffbeb', border: '#fde68a', color: '#92400e', icon: 'âš ' },
    INVALIDATED: { bg: '#fef2f2', border: '#fecaca', color: '#991b1b', icon: 'âœ—' },
  ...
  return `<div style="padding:6px 10px;background:${cfg.bg};border:1px solid ${cfg.border};
    ...
    <span style="margin-left:auto;white-space:nowrap;color:var(--text-muted)">${daysAgo}d old Â· ${result.model||''}</span>
  </div>`;
}

// â”€â”€ REC HISTORY FILTER STATE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ History rec card â€” same layout as the pending card, outcome-aware â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  return `<div class="card">
    ...
    <div style="background:var(--bg-secondary);border-radius:var(--radius-md);padding:12px 14px;margin-bottom:14px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
        ...
        <div style="display:flex;gap:8px;align-items:center">
          ...
          ${hasFilters ? '<button class="btn btn-sm" onclick="resetRecHistoryFilter()">âœ• Clear filters</button>' : ''}
          <button class="btn btn-sm btn-danger" onclick="clearRecHistory()">ðŸ—‘ Clear history</button>
        </div>
      </div>
      <div style="display:flex;flex-wrap:wrap;gap:10px;align-items:flex-end">
        ...
        <div>
          ...
          <select onchange="setRecHistoryFilter('outcome',this.value)" ${selStyle}>
            ...
            <option value="win"     ${f.outcome==='win'?'selected':''}>Won âœ“</option>
            <option value="loss"    ${f.outcome==='loss'?'selected':''}>Lost âœ—</option>
        ...
        <div>
          ...
          <select onchange="setRecHistoryFilter('minConf',Number(this.value))" ${selStyle}>
            ...
            <option value="55" ${f.minConf===55?'selected':''}>â‰¥55%</option>
            <option value="65" ${f.minConf===65?'selected':''}>â‰¥65%</option>
            <option value="75" ${f.minConf===75?'selected':''}>â‰¥75%</option>
            <option value="85" ${f.minConf===85?'selected':''}>â‰¥85%</option>
          </select>
        </div>
        <!-- Sort -->
        <div>
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:12px">
      ...
      ${execCount > 0 ? `<div style="background:var(--bg-secondary);padding:8px 12px;border-radius:var(--radius-md);font-size:12px">
        <span class="text-muted">Win rate</span> <strong>${wonCount+lostCount > 0 ? fmt(wonCount/(wonCount+lostCount)*100,0)+'%' : 'â€”'}</strong>
      </div>` : ''}
    ...
    ${filtered.length === 0
      ? `<div class="empty-state" style="padding:2rem"><div class="empty-icon">â—†</div><p>${state.recHistory.length === 0 ? 'No recommendation history yet.' : 'No records match your filters.'}</p></div>`
...
function updateExecTotal(recId) {
  ...
  if (!el) return;
  if (!qty || !price) { el.textContent = 'â€”'; return; }
  const rec = state.recommendations.find(r => r.id === recId);
...
function _showPreTradeChecklist(recId, onConfirm) {
  ...
  // â”€â”€ Confidence-floor breach: dedicated hard gate, not just one of six items â”€â”€
  ...
  if (lowConfWarning) {
    ...
    lowConfBanner = `<div style="background:#fee2e2;border:2px solid #dc2626;border-radius:6px;padding:10px 12px;margin-bottom:12px">
      <div style="font-size:13px;font-weight:800;color:#991b1b;text-transform:uppercase;letter-spacing:0.3px">âš  Below Confidence Floor</div>
      <div style="font-size:12px;color:#991b1b;margin-top:4px;line-height:1.5">
        ...
        Recs in this range have historically underperformed â€” executing means overriding that empirical signal on your own judgment.
      </div>
    </div>`;
  }
  ...
  if (isReducing && rec) {
    ...
    if (urgentParcels.length) {
      ...
      cgtWarning = `<div style="background:#fef3c7;border:1px solid #f59e0b;border-radius:6px;padding:8px 12px;margin-bottom:12px;font-size:12px;color:#92400e">
        <strong>âš  CGT Discount Warning</strong><br>
  ...
  const items = [
    ...
    { id: 'chk-avg',      label: isReducing
        ? 'I\'m not exiting in panic â€” the thesis has actually changed'
        : 'I\'m not averaging into a losing position without a plan' },
    { id: 'chk-reason',   label: isReducing
        ? 'This exit is to harvest profits or manage risk â€” not panic or FOMO'
        : 'This entry fits a planned strategy â€” not FOMO or impulse buying' },
  ];
  ...
  dialog.innerHTML = `
    <div style="padding:20px 22px">
      ...
      <div style="font-size:12px;color:var(--text-muted);margin-bottom:14px">${rec ? rec.action + ' ' + rec.ticker : ''} â€” tick at least 4 to proceed</div>
      ${lowConfBanner}
      ${cgtWarning}
      ...
      <div style="display:flex;gap:8px;justify-content:flex-end">
        ...
        <button id="checklist-confirm-btn" onclick="_checklistProceed('confirm')" class="btn btn-sm btn-success" disabled>âœ“ Execute Trade</button>
      </div>
    </div>`;
...
// â”€â”€ Â§9.4: "Why did I get this rec?" traceability modal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Pure UI assembly â€” every input already lives on the rec object as an audit
...
function showRecTraceability(recId) {
  ...
  const genRows =
    row('Generated', `${r.generatedAt || r.date || ''}${r.regime ? ` Â· regime: <strong>${r.regime}</strong>` : ''}`) +
    row('Source', r._source === 'local' ? 'ðŸ”’ Local Ollama model' : 'Claude (tool-schema output)') +
    row('Action', `${r.action} ${r.ticker}${r.isWatchlist ? ' â˜…watchlist' : ''}`);
  const confRows =
    row('Claude confidence', pct(origConf)) +
    (r._calibApplied
      ? row('Calibration applied', `${r._calibApplied.adj > 0 ? '+' : ''}${(r._calibApplied.adj * 100).toFixed(0)}pp (band ${r._calibApplied.band || 'â€”'}) â†’ ${pct(r.confidence)}`)
      : '') +
    row('Ensemble (Â½AI + Â½indicator)', pct(r.ensembleConfidence));
  const sizingRows =
    row('Quant engine', r._quantEngine
      ? `qty=${r.qty}${r._constraintBinding ? ` Â· binding constraint: ${r._constraintBinding}` : ''}`
      : (r._fallbackSized
        ? `declined â€” fallback min-trade sizing applied (qty=${r.qty}; see warnings)`
        : (r._exitSized ? null : 'AI values kept (engine skipped)'))) +
    row('Exit sizing', r._exitSized ? (r._exitSized === 'full' ? 'SELL â€” full holding' : `TRIM â€” ${r._exitSized} of holding (weightGuidance midpoint)`) : null) +
    row('Risk / reward', r.riskAUD != null ? `$${fmt(r.riskAUD)} / $${fmt(r.rewardAUD)} (R:R ${r.rrRatio ?? 'â€”'})` : null) +
    row('Pre-earnings widen', r._preEarningsAdj ? 'stop Ã—1.3 â€” earnings â‰¤14d' : null) +
    row('Correlation', r._corrNote || null) +
    row('Heat budget', r._budgetScaled ? (r._budgetNote || 'scaled to fit risk budget') : null) +
    row('VaR adjustment', r._varAdjusted ? `qty Ã—${r._varMult} (VaR1d ${r._varMult === 0.5 ? '< âˆ’5.0%' : '< âˆ’3.5%'})` : null) +
    row('Stop repaired', r._stopRepaired ? `ATR floor applied (${r._stopRepairedMult}Ã—ATR)` : null) +
    row('Stop widened', r._stopWidened ? `$${(r._stopWidenedFrom ?? 0).toFixed(2)} â†’ $${(r.stopLoss ?? 0).toFixed(2)} (${r._stopWidenedRegime} regime)` : null) +
  ...
  const thesisRows =
    ...
    (r.primary_driver ? row('Exit tags', `${r.primary_driver}${Array.isArray(r.secondary_factors) && r.secondary_factors.length ? ' Â· ' + r.secondary_factors.join(', ') : ''}${r.urgency ? ' Â· ' + r.urgency : ''}`) : '');
  ...
  dialog.innerHTML = `
    <div style="padding:18px 20px">
      <div class="flex-between" style="margin-bottom:12px">
        <div style="font-size:15px;font-weight:700">â„¹ï¸ Why this rec â€” ${r.action} ${r.ticker}</div>
        <button class="btn btn-sm" onclick="document.getElementById('rec-trace-dialog').close()">âœ•</button>
      </div>
...
function _checklistProceed(mode) {
  ...
  // non-negotiable gate â€” there is no override path that skips it (the
...
function _showThesisReview(thesisText, learningId, ticker) {
  ...
  dialog.innerHTML = `
    <div style="padding:20px 22px">
      <div style="font-size:15px;font-weight:700;margin-bottom:4px">Thesis Review â€” ${ticker}</div>
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        <button id="tr-held"    class="btn btn-sm btn-success"  style="flex:1">âœ“ Thesis Held</button>
        <button id="tr-partial" class="btn btn-sm"              style="flex:1;background:#d97706;color:#fff;border-color:#d97706">âš¡ Partially</button>
        <button id="tr-broken"  class="btn btn-sm btn-danger"   style="flex:1">âœ— Thesis Broken</button>
      </div>
  ...
  const submit = verdict => {
    ...
    const noteTag = verdict === 'held' ? '[THESIS: held âœ“]' : verdict === 'partial' ? '[THESIS: partial âš¡]' : '[THESIS: broken âœ—]';
...
function trailStop(recId) {
  ...
  if (!price || !atr) {
    toast(`No live price/ATR for ${rec.ticker} â€” refresh signals first`, 'error');
    return;
  }
  ...
  if (!wouldTighten) {
    toast(`Stop already tighter than ${mult}Ã—ATR trail (current $${rec.stopLoss.toFixed(3)} vs trail $${newStop.toFixed(3)})`);
    return;
  }
  ...
  const confirmed = confirm(
    ...
    `  New stop:     $${newStop.toFixed(3)} (${mult}Ã—ATR @ $${price.toFixed(3)})\n\n` +
    `Proceed?`
  );
  if (!confirmed) return;
...
// Snapshot live technicals at fill time â€” shared shape for both the BUY/TOP_UP
// entry capture and the SELL/TRIM exit capture, so the Journal drawer can show
// an entryâ†’exit delta on the same fields. Returns null when no live signal
...
async function markExecuted(id, execPrice, execFee, execQty, execAccount) {
  ...
  if (_execBtn) {
    _execBtn.textContent = 'âœ“ Executed';
  ...
  // Ensure live signals are available before snapshotting â€” state.liveSignals
  ...
  if (!state.liveSignals?.[rec.ticker]?.rsi_14) {
    ...
    } catch { /* offline â€” proceed without signals; snapshot will be null */ }
  }

  // Same fallback for regime â€” a trade executed before the Dashboard's
  ...
  if (!state.currentRegime && typeof fetchAndClassifyRegime === 'function') {
    try { await fetchAndClassifyRegime(); } catch { /* offline â€” proceed without regime */ }
  }

  // Regime live at the moment this transaction is actually filled â€” distinct
  ...
  // normalized (_normTicker strips an optional ".AX" suffix and uppercases) â€”
  ...
  if (execAccount && _distinctAccts.includes(execAccount)) {
    ...
    // _distinctAccts.length > 1) â€” always trust this over any guess, since
    ...
    console.warn(`[markExecuted] ${rec.ticker} is held in multiple accounts (${_distinctAccts.join(', ')}) â€” ` +
  ...
  let _saleDisposals = null;     // raw disposals â€” used below for blended entryPrice/holdDays on the learning-loop payload
  if (isReducing) {
    ...
    if (holding && holding.shares >= qty) {
      ...
      // generated the SELL/TRIM rec. One snapshot for the whole execution â€”
      ...
      // One Trade Journal row per parcel actually matched â€” not one aggregate
      ...
      toast(`${rec.action} ${rec.ticker}: not enough shares held â€” logged but portfolio unchanged.`, 'warn');
    }
  } else {
    // BUY or TOP_UP â€” open/add to a position
  ...
  // which is reliable regardless of which UI flow created them â€” Day
  // Trading's own executeDayTrade()/executeIntradayTrade() included) â€” only
  ...
  // state.intraday.openPositions[] â€” neither of which markExecuted() (the
  // generic Recommendations close path) otherwise touches.
  //
  // This used to gate on `sellAccount === 'trading'` â€” but sellAccount is a
  ...
  // sale is ground truth, independent of which account the guess landed on â€”
  ...
  // Save to recHistory â€” include all execution details so backtest can use them
  // SELL/TRIM that fully disposed a holding already know their P&L (realizedPnl,
  // set above) â€” outcome must reflect win/loss/breakeven immediately, not stay
  ...
  if (state.serverOk) {
    ...
    // disposal's own row) â€” tradeEntry.entryPrice now reflects only ONE
    ...
    // semantics â€” the oldest parcel touched by this sale.
    ...
    if (rec._learningId) {
      // Existing learning event â†’ update with full outcome context
      ...
      if (isReducing && outcome) {
        // Scoped by sellAccount â€” an unscoped lookup would return the FIRST
        ...
        if (positionClosed) {
          ...
          // #2b: thesis review â€” surface thesis from any parent BUY rec
      ...
      fetch(`${API}/api/learning/log`, {
        ...
        body: JSON.stringify({
          ...
          ...(tradeEntry.exitSignals ? { exit_signals_json: JSON.stringify(tradeEntry.exitSignals) } : {}),
          notes:               'Manually imported position â€” no prior BUY event logged',
        }),
        ...
        if (res.id) {
          ...
          // Postmortem and skill scoring are manual-only â€” use Debate Engine on Learning page
        }
      }).catch(() => {});
    }
  }
  ...
  toast(`${rec.ticker} ${rec.action} ${qty} @ $${fmt(tradePrice)} (fee $${fmt(fees)}) â€” logged`, 'success');
  scheduleSave();
  renderPage();
}
function markSkipped(id) {
  ...
  // scenarios, bull/bear, traceability, etc.) â€” not just the minimal subset.
...
function submitFeedback(recId) {
  ...
  if (rec) {
    ...
    scheduleSave();
    toast(text ? `Feedback saved â€” will be included in next analysis run` : 'Feedback cleared', 'success');
  }
}
...
function renderPositionSizer(rec) {
  ...
  const riskPct = 1.0;   // default 1% risk â€” user can change via input
  ...
  <details style="margin-bottom:10px" id="${sId}-details">
    <summary style="cursor:pointer;padding:7px 10px;background:var(--bg-secondary);border-radius:var(--radius-md);border:0.5px solid var(--border-light);font-size:12px;font-weight:600;color:var(--text-secondary);user-select:none;list-style:none;display:flex;align-items:center;gap:6px">
      <span>ðŸ“</span> Position Sizing Calculator
...
function sizerOutput(qty, posValue, posWeight, maxLoss, potGain, rr, fees, nw) {
  ...
  return [
    tile('Suggested Qty', qty > 0 ? `${qty} shares` : 'â€”', qty > 0 ? `$${fmt(posValue)} total` : 'check stop-loss'),
    tile('Position Size', posValue > 0 ? `$${fmt(posValue)}` : 'â€”', `${fmt(posWeight, 1)}% of net worth`, overweight ? '#ef4444' : undefined),
    tile('Max loss if stopped', maxLoss != null ? `$${fmt(maxLoss)}` : 'â€”', maxLoss != null ? `inc. $${fees} brokerage` : 'set stop-loss'),
    tile('Potential gain', potGain != null ? `$${fmt(potGain)}` : 'â€”', potGain != null ? 'if target hit (2Ã— brokerage)' : 'set target'),
    tile('R:R ratio', rr != null ? `${rr.toFixed(1)} : 1` : 'â€”', rr != null ? (rr >= 2 ? 'âœ“ Acceptable' : 'âš  Below 2:1 â€” avoid') : '', rrColor),
    overweight ? tile('âš  Position limit', `Max ${maxPosPct}% rule`, `Reduce qty to stay within limit`, '#ef4444') : '',
  ].join('');
}
  ...
  return `
    <div style="display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:var(--warn-bg);border:1px solid var(--warn);border-radius:var(--radius-md);padding:10px 14px;font-size:12px;color:var(--warn);display:flex;align-items:center;justify-content:space-between">
        <span>Custom rules active â€” defaults have been modified. These are applied on every analysis run.</span>
        <button class="btn btn-sm" onclick="resetRecRules()" style="white-space:nowrap;color:var(--warn)">â†º Reset all</button>
      </div>` : `
      <div style="background:var(--bg-secondary);border-radius:var(--radius-md);padding:10px 14px;font-size:12px;color:var(--text-muted)">
        Using default rules. Edit any value below to override â€” changes are applied immediately on the next run.
      </div>`}
      ...
      <div class="card">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:14px;margin-top:8px">
          ${nf('Min R:R ratio', 'minRrRatio', 1.0, 5.0, 0.5, 'Minimum reward:risk â€” recs below this are rejected')}
          ${nf('Stop ATR multiple', 'stopAtrMultiple', 0.5, 5.0, 0.5, 'Stop-loss = entry âˆ’ NÃ—ATR14')}
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:14px;margin-top:8px">
          ${nf('Same-ticker window (days)', 'sameTickerWindowDays', 1, 30, 1, 'No BUYâ†’SELL flip on same ticker within this window')}
      ...
      <div class="card">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:14px;margin-top:8px">
          ${nf('VaR1d threshold 1 (%)', 'highVarThreshold1', -20, 0, 0.5, 'VaR1d below this â†’ reduce qty by 25%')}
          ${nf('VaR1d threshold 2 (%)', 'highVarThreshold2', -20, 0, 0.5, 'VaR1d below this â†’ reduce qty by 50%')}
          ${nf('MaxDD90d threshold (%)', 'highDdThreshold', 5, 60, 1, 'Max drawdown above this â†’ mandatory stop-loss note')}
          ${nf('Composite risk high score', 'compositeRiskHighScore', 30, 95, 1, 'Risk score above this â†’ raise min confidence')}
          ${nf('Composite risk very-high score', 'compositeRiskVeryHighScore', 50, 100, 1, 'Risk score above this â†’ no new BUY positions')}
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:14px;margin-top:8px">
          ...
          ${nf('CGT window end (months)', 'cgtHoldMonthsMax', 10, 18, 1, 'End of CGT discount window â€” flag any SELL before this')}
        </div>
      </div>
      ...
      <div class="card">
        ...
        <p class="text-xs text-muted" style="margin-bottom:10px">Appended verbatim to every analysis run as additional AI instructions. Plain English only â€” no JSON.</p>
      ...
      <div class="card" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px">
        <div>
          ...
          <div class="text-xs text-muted">Confidence floor is enforced client-side Â· all other rules injected into AI user message</div>
        </div>
        <div class="flex-row">
          <button class="btn btn-sm" onclick="resetRecRules()">â†º Reset to defaults</button>
          <button class="btn btn-primary btn-sm" onclick="switchRecTab('run');setTimeout(runAnalysisFromPanel,50)">â–¶ Run Analysis Now</button>
        </div>
      </div>
