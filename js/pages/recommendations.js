  ...
  return summaryBanner + recs.map(r => {
    ...
    <div class="rec-card" id="rec-${r.id}">
      ...
      <div class="flex-between mb-1">
        <div class="flex-row">${actionBadge(r.action)}<strong style="font-size:15px">${r.ticker}</strong>${livePxBadge}${entryStatusTag}<span class="text-xs">${r.sector||''}</span>${_sectorConcentrationWarning(r)}${r._corrNote?`<span class="badge" style="background:#fef3c7;color:#92400e;border:none" title="${escapeHTML(r._corrNote)}">⚡ Corr</span>`:''}${r._budgetScaled?`<span class="badge" style="background:#fff7ed;color:#c2410c;border:none" title="${escapeHTML(r._budgetNote||'')}">⚠ Budget</span>`:''}${r._preEarningsAdj?`<span class="badge" style="background:#eff6ff;color:#1d4ed8;border:none" title="Stop distance widened 30% — earnings within 14 days">📅 Pre-earnings</span>`:''}${r.isWatchlist?'<span class="badge" style="background:#ede9fe;color:#6d28d9;border:none">★ Watchlist</span>':''}${r._source==='local'?'<span class="badge" style="background:#fff7ed;color:#92400e;border:none" title="Generated by local Ollama model — BUY/HOLD only">🔒 Local</span>':''}${r._stopTrailed?`<span class="badge" style="background:#f0fdf4;color:#15803d;border:1px solid #86efac" title="Stop trailed at ${r._stopTrailedAt||''}">📍 Trailed</span>`:''}<span class="badge badge-pending">Pending</span><span class="text-xs text-muted">Generated ${r.generatedAt||r.date}</span></div>
        <div class="flex-row">
