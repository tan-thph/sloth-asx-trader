...
python3 asx_server.py</pre>
      ...
      <div style="margin-top:14px;padding-top:12px;border-top:0.5px solid var(--border-light)">
        ...
        <div style="display:flex;align-items:center;gap:8px">
          <input type="number" min="0.5" max="1.0" step="0.01"
            ...
            style="width:80px;padding:5px 8px;border-radius:var(--radius-md);border:0.5px solid var(--border-medium);background:var(--bg-primary);color:var(--text-primary);font-size:13px">
          <span class="text-xs text-muted">hard-block BUY · soft gate 10pp below</span>
        </div>
        <div class="text-xs text-muted mt-1">
          BUY recs with |ρ| at or above this threshold vs any existing holding are hard-blocked —
          near-perfect correlation provides no diversification benefit. Soft gate (−30% size) fires
          10pp below (e.g. 0.75 when threshold is 0.85).
        </div>
      </div>
    </div>
    <div class="card">
    ...
    <div class="card">
      ...
      <div style="display:grid;grid-template-columns:auto 1fr;gap:4px 16px;font-size:12px;align-items:baseline">
        ...
        <span class="text-muted">DB in git</span>
...
async function loadSettingsAppInfo() {
  ...
  // DB git status — last committed date for asx_trader.db
  try {
    ...
    if (gitEl) {
      if (gd.ok) {
        ...
        gitEl.style.color = gd.has_uncommitted ? '#d97706' : '';
      } else {
