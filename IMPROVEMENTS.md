...
## Sprint 72 — note.txt triage (started 2026-06-20)

Status legend: ✅ done · 🔧 ready to implement (root cause confirmed) · 🔍 needs investigation · ❓ needs user clarification · ⏭ deferred (design decision, not a bug)

### Incident (found during triage, not in note.txt) — ✅ RESOLVED

**`asx_trader.db` was corrupted** (`PRAGMA integrity_check` failed with dozens of "invalid page number" errors; `test_app.py` couldn't even import because `init_db()` threw `database disk image is malformed`). Consistent with note.txt #34 ("Day Trading history corrupted after opening `asx_trading.html` directly instead of via ngrok").

- ✅ Recovered via SQLite backup-API clone + rowid-bisection salvage on tables that still errored after cloning, then reinserted all salvaged rows into a freshly `init_db()`'d schema (plus `announcement_engine.init_db()` / `news_engine.init_news_tables()` for the two tables `db.py` doesn't own). **Zero loss** on `trade_journal` (39), `rec_history` (17), `portfolio` (17), `cash` (1), `settings` (22), `blob_store` (21 — watchlist/alerts/screeners/target allocations all intact). Partial loss on `ai_learning_events` (34/65), `ai_call_log` (10/23), `news_items` (23/233) — logs/caches, not source-of-truth. Full loss on `announcements` (0/60) and `dividend_cache` — both regenerate automatically from ASX scrape / yfinance on next run.
- ✅ Verified: `PRAGMA integrity_check` → `ok` on the rebuilt DB; `python test_app.py` now runs (758 tests, was failing at import before).
- ✅ Kept `asx_trader.db.corrupted-20260620` and the pre-existing `asx_trader.db.bak-20260613` on disk as safety copies.
- ✅ Fixed stale test `test_portfolio_token_cap_raised` (expected `portfolio: 10000` in `claude-client.js`, code has `12000` since a later sprint raised it further) — this is also note.txt #16. `test_app.py` now passes except the pre-existing, environment-only `test_vitest_suite_passes` (vitest isn't on PATH in this shell — see project memory `test-runner-env.md`).
- 🔍 **Root cause of the corruption itself is still open** — recommend never opening `asx_trading.html` as a local `file://` while a server process also holds `asx_trader.db` open (two independent writers without coordinated locking can corrupt SQLite even in WAL mode). Always go through `http://localhost:5000` or the ngrok URL.

### Tier 1 — trivial/small, root cause confirmed, low risk — ✅ ALL DONE 2026-06-20

| # | Issue | File(s) | Root cause | Fix |
|---|---|---|---|---|
| 8 | RBA board meeting dates wrong for Aug/Sep/Nov/Dec 2026 | `js/pages/dashboard.js` (`_RBA_MEETINGS_2026`) | Stale/mistranscribed hardcoded array | ✅ Corrected: Aug 19→11, Sep 30→29, Nov 04→03, Dec 09→08 |
| 9 / 31 | Account tag (Personal/Super/Trading) selected state invisible in light theme | `js/pages/portfolio.js:97` + 7 other files | Inline/CSS references to undefined `var(--accent)` (only `--accent-primary` etc. are defined) — invalid declaration dropped, white-on-white in light theme | ✅ Fixed all 11 occurrences across portfolio.js, signals.js, compare.js, news.js, announcements.js, settings.js → `var(--accent-primary)` |
| 7 / 28-notif | "Scanner: N high-score tickers" notification click does nothing | `js/notifications.js` (`_NOTIF_NAV` map) | `'scan'` category isn't a key in `_NOTIF_NAV`, so `_notifNavigate()` no-ops (confirmed: `alertScanComplete` is Market-Scanner-only, no ambiguity) | ✅ Added `scan: 'scanner'` to `_NOTIF_NAV` |
| 6a / 27 | `/api/risk` yfinance pct_change FutureWarning | `routes/market.py` (3 sites: ~114, ~288, ~595) | `pct_change()` called without `fill_method=None` | ✅ Added `fill_method=None` to all 3 call sites |
| 6b / 27 | `/api/risk` can return invalid JSON (`Unexpected token 'N'...`) | `routes/market.py` `portfolio_risk()` (~571-644) | NaN metrics serialize as a bare `NaN` token via `jsonify`, which browser `JSON.parse()` rejects | ✅ Wrapped every response metric in the existing `safe_float()` helper (reused from `indicators.py`, already imported) |
| 16 | `test_app.py` had 1 failure | `test_app.py` | Stale assertion vs current code | ✅ Done (see Incident section) |

### Tier 2 — small/medium, root cause confirmed — ✅ ALL DONE 2026-06-20

| # | Issue | File(s) | Root cause | Fix |
|---|---|---|---|---|
| 1 | Learning Loop tag click rebuilds the whole page (feels like a reload) | `js/pages/learning.js` | No targeted-DOM-patch path for a tag change — always called `showPage('learning')` | ✅ Hoisted tag-cell renderer to module scope, added `id="ll-tagcell-${id}"` DOM hook; `tagLearningEvent()` now patches just that cell + updates the cached event object, no more full-page rebuild |
| 2 | BB%B sometimes shows >100% | `indicators.py` (raw `%B`, correctly unclamped) | **Not a bug.** Exhaustive search of every render site (signals/watchlist/scanner/journal/recommendations/assistant pages) confirms `bb_pct_b` is *always* shown as plain text (`${pct}%`), never as a width-bound progress bar — the only such bar in the codebase (`conf-fill`) is driven by `confidence`, unrelated. 101% is the mathematically correct "price closed above the upper band" breakout signal. | ⏭ No change made — working as intended |
| 4 / 26 | Active Setups entries (e.g. EVT.AX) vanish after re-running Day Trade Scan | `js/day-trading-analysis.js` (~506-524), `js/pages/day-trading.js` | `kept` filter dropped any still-pending universe-sourced rec that didn't requalify under the new scan | ✅ Previously-pending recs not reappearing in the new scan are now kept with `_stale: true` / `_staleScans` counter (capped at 3 consecutive missed scans, mirroring the existing `_dtDismissStaleRecs(3)` precedent), tickers that DO reappear get the fresh data. Added an amber "⚠ NOT IN LATEST SCAN" badge distinct from the existing age-based stale badge. |
| 5 | Day Trading History tab: no filter, no $ P&L | `js/dt-training.js` (`renderDtHistoryTab`, `_dtHistRow`) | No filter UI existed; row renderer only showed `pnl_pct` (backend `trade_snapshots` table has no dollar field) | ✅ Added ticker text filter + outcome dropdown (win/loss/breakeven/open), styled to match `journal.js`'s existing filter pattern. Added a $ P&L column computed client-side from `entry_price`/`exit_price`/`qty` (direction-aware for the rare non-BUY case). Table now uses `.tbl-stack` + `data-label` per gotcha #37. |

### Tier 3 — ✅ MOSTLY DONE 2026-06-20

**Code fixes shipped:**

| # | Issue | File(s) | Root cause | Fix |
|---|---|---|---|---|
| 25 | ORG/GMG unexplained in Day Trading History → Swing, 17 Jun; "is Intraday converted to Swing?" | `day_trade_history.db` data (confirmed via direct query) + `js/dt-training.js` | ORG/GMG are **shadow log records** (`record_type='shadow'`, `shadow_reason='conf_floor'`) written by the ML bias-correction pipeline when a setup's confidence was below the action floor — never real trades, never meant to have a close. Confirmed separately: Intraday is never converted to Swing — both `dtSaveSnapshot(..., 'swing', ...)` call sites live only in the Setups/swing scan flow. The actual bug: the History tab fetched `/api/daytrading/history` with no `record_type` filter, so shadow rows displayed indistinguishably from real trades. | ✅ History now defaults to `record_type=executed` (shadow excluded by default); added an opt-in "🔬 Show shadow records" checkbox that renders a distinct purple "SHADOW" badge, shows `r_multiple`/`shadow_reason` instead of a fake close, and explains the semantics in a tooltip. |
| 14 | Watchlist Trends show "neutral" for every ticker | `indicators.py:601-602` | `bullish_count`/`bearish_count` used `v is True`/`v is False` identity checks against `trend_signals` values, which are `numpy.bool_` — `numpy.bool_(False) is False` is `False` in Python, so both counters were **always 0**, forcing `trend_score=0.0` → always "neutral". Same family as gotcha #21 (numpy bool) but via identity comparison instead of JSON serialization, so it slipped past the existing safeguard. Frontend (`watchlist.js`) was fine end-to-end. | ✅ Cast to native `bool()` before the identity check. Empirically verified CBA/BHP now show bearish, CSL shows bullish (was 0.0/neutral for all three before the fix). |
| 18 | Regime doesn't save for some Day Trading trades | `js/dt-training.js` (`dtSaveSnapshot()`) | Snapshot writer read `state.currentRegime.regime` **live at execution time** instead of the regime already correctly stamped on the rec at scan time (`rec.regime`, set in `_dtBuildRecs()`). Regime could flip during the up-to-3-day pending window, or be null if execution happened before `fetchAndClassifyRegime()` resolved. Mirror image of the bug fixed in commit `4bf219a` for the portfolio path. | ✅ `dtSaveSnapshot()` now prefers `rec.regime` (when set, not the `'unknown'` placeholder) over a live re-read, matching the 4bf219a pattern. Both call sites in `day-trading-analysis.js` fixed by the one change. |
| 22 | Position-weight hard 15% cap should include cash, not just shares value | `js/quant-engine.js:103-108`, `js/prompts.js` | The max-position-% sizing cap used `allocatedCash * maxPosPct / price` (cash-only denominator) while the risk-% and Kelly constraints in the same function already correctly used `capital` (cash + holdings value / net worth) — only the position cap was inconsistent, letting it silently expand once a portfolio held substantial positions. | ✅ Position cap now uses `capital` (net worth) like the other two constraints. Updated `prompts.js` text ("Max single position: 15% of total net worth") so Claude's qualitative reasoning matches the engine's actual denominator. Updated a Vitest fixture-dependent test that asserted the old denominator. **Needs a human to run `npm run test:js` once vitest is available** — not on PATH in this environment. |
| 23 | AI Call #20: GMG emitted in `recs[]` despite "REJECT" reasoning; SHL action=SELL contradicts "deferring" reasoning; AI-supplied `netProfit` doesn't reconcile | `js/response-validator.js`, `js/analysis.js` | No deterministic check existed for "reasoning says reject/defer but action is non-HOLD" contradictions; AI-supplied `netProfit`/`expectedProfit`/`riskAUD`/`rewardAUD` weren't stripped at the validator layer (the live UI path already overwrites them downstream, but other callers wouldn't have been protected). | ✅ Added a `reject-defer-contradiction` business rule (phrase-matches reasoning text against reject/defer language co-occurring with a non-HOLD action) that's unfixable-by-design, triggering the existing repair-retry loop; `analysis.js` also auto-demotes any rec that still fails this check after retry, dropping it from `recs[]` with a logged reason. Added `_stripUntrustedAiFields()` to scrub the 4 AI-supplied P&L fields from every validated rec. 8 new tests added. (Issue 3, the catastrophe-clause misapplication, was left as a free-text judgement call — not cleanly checkable deterministically.) |
| 30 | Portfolio Forward Yield (Morning Macro) disagreed with Portfolio page's "yield on cost" | `js/pages/macro.js`, `js/pages/portfolio.js` | **Not a calculation bug** — two correctly-computed, differently-denominated metrics: Portfolio's "yield on cost" divides by original purchase cost basis; Morning Macro's "Forward Yield" (and the Dividend Income Schedule's "Div yield") divides by current market value. Both numerators are identical; only the denominator differs, which is why they disagree (holdings have appreciated). | ✅ Labeling only: added "(on market value)" suffix + cross-referencing tooltips on both metrics so the legitimate distinction is self-explanatory rather than confusing. |
| 13 | Trade Journal always shows "No thesis recorded." even when an explanation is visible below; tag meanings (`validated`/`invalidated`/`irrelevant`) undocumented | `js/pages/journal.js`, `js/analysis.js` | Not a field-mismatch or timing bug — `trade_thesis` is genuinely almost never populated for AI-sourced trades: `logRecsToLearningLoop()` sends `rationale_summary` (Claude's reasoning) to the backend but never `trade_thesis` (that field is only set when a human manually types into an opt-in thesis textbox at execution time). So the "explanation below" the user saw *was* `rationale_summary`, just not recognized as a thesis. | ✅ Journal drawer now falls back to `rationale_summary` (labeled as "Claude's rationale" to distinguish from an explicit thesis) when no thesis was recorded, and suppresses the duplicate line below. Added hover tooltips on each verdict badge explaining what `validated`/`invalidated`/`irrelevant` actually mean, derived from the real `computeThesisDrift()` logic (per-driver: mean_reversion/momentum_breakout/trend_pullback are technically verifiable; fundamental_value/macro_tailwind are always `irrelevant` since they're not technically verifiable). |
| 3 | high_60d/low_60d computed from adjusted prices, not nominal — Fibonacci day-trade zones use the wrong basis | `indicators.py` (`analyse_ticker()`) | Confirmed: only one `auto_adjust=True` fetch existed; no separate nominal-price fetch for these two fields specifically. | ✅ Implemented exactly as originally requested: added a second `auto_adjust=False` fetch (with Stooq fallback, and the same `_drop_forming_bar`/`_sanity_check` guards as the primary series) feeding ONLY `high_60d`/`low_60d`; every other indicator (RSI/MACD/SMA/Bollinger/etc.) still reads the adjusted series, unchanged. `composite_signal` left as-is (its removal/relabeling was a separate lower-priority cosmetic suggestion, not implemented). **Tradeoff:** one extra yfinance network call per `analyse_ticker()` invocation (absorbed by the existing 5-min route-level `ttl_cache`, so the marginal cost is per cache window, not per UI refresh). |

**Informational answers (no code change needed):**

| # | Question | Answer |
|---|---|---|
| 15 | gunicorn doesn't start with ngrok | `gunicorn` is POSIX-only by design (uses `fcntl`) and was never going to work on Windows — `waitress_server.py` is the documented Windows alternative. ngrok cleanup-on-shutdown + port-probing was already fixed in commit `08142fd` (2026-06-14). If this is still failing, confirm you're using `waitress_server.py` on Windows, not trying to run gunicorn directly. |
| 19 | How does the app handle the same ticker under different accounts (Personal/Super/Trading)? | Correctly — `mergedPortfolio()` (`js/utils.js`) keys holdings by `ticker + account`, so e.g. WES held personally and WES bought in Super are tracked as fully separate position entries, never merged. |
| 20 | How is Success Patterns determined/captured? | `success_tags` are populated by `POST /api/debate/skill` (tags wins with what went right) combined with a deterministic "dominant success tag" nudge computed in `_calib_compute()` (`routes/learning.py`) — surfaced as the `success_patterns` field in `/api/learning/stats` and the Success Patterns card on the Learning page. |
| 21 | Do Day Trading rules (`DT_FILTER`/`DT_AI_PARAMS`) apply to Intraday or only Setups? | Only the Setups (swing) tab. Confirmed via grep: `js/intraday-strategy.js` and `routes/intraday.py` never reference `DT_FILTER`/`DT_AI_PARAMS`/`filterParams`/`aiParams` — Intraday has its own separate hardcoded thresholds (entry-window gate, VWAP/RSI/score gates in `routes/intraday.py`). |
| 24 | Day Trading Setups tickers have `.AX` suffixes leaking into Portfolio/Journal? | Inconclusive from static search — no `.AX`-stripping or `.AX`-appending logic found in `day-trading-analysis.js`/`asx-universe.js`, suggesting Day Trading tickers are normally stored bare (no suffix) like the rest of the codebase. Could not reproduce the leak from code reading alone; if it recurs, a specific ticker/screenshot example would let this be re-investigated precisely. |
| 28 | Market Scanner vs Day Trading — how do strategies differ? | Market Scanner (`routes/scanner.py`) is a broad universe technical screen with sector diversification, used for general stock-picking. Day Trading Setups (`day-trading-analysis.js` + backend) and Intraday (`routes/intraday.py`) are specifically tuned for short-hold mean-reversion/momentum setups with their own entry-window/VWAP/setup-score gates — a different strategy family, not a subset of the Scanner. |
| 32 | How are News/Announcements fetched into the Claude API call? | `js/analysis.js` calls `GET /api/news/brief` (LLM-classified articles, last 2 days) and `GET /api/announcements/brief` (last 3 days, max 10) per ticker, and injects both as `NEWS SIGNALS` / announcement-context blocks directly into the user message sent to Claude — confirmed both are wired in, not dropped. |
...
| Item | File(s) | Root cause | Fix |
|---|---|---|---|
| Account-blind SELL matching (note.txt §8 Part A) | `js/utils.js`, `js/pages/recommendations.js` | `getPortfolioHolding()` matched by ticker only (no account filter); `markExecuted()` called `applySellToPortfolio()` without an account arg, which silently defaults to `'personal'` — so selling a `'trading'`-only holding via the generic Recommendations flow proceeded (journal written, rec marked executed) without ever decrementing real portfolio shares/parcels. Silent data-corruption bug. | ✅ `markExecuted()` (`recommendations.js` ~1282-1318) now resolves the unambiguous account for the ticker being sold (looks up holdings across all accounts; if exactly one match, uses it) and passes it explicitly to `getPortfolioHolding()`/`applySellToPortfolio()`. Ambiguous multi-account case (>1 holding of the same ticker) keeps the old `'personal'` default but now emits a `console.warn` so it's visible instead of silent. `tradeEntry.account` is also stamped. |
| Day Trading state desync on generic close (note.txt §8 Part A cont.) | `js/pages/recommendations.js` | Closing a Day-Trading-sourced position via the generic Recommendations UI never updated `state.dayTrading.recommendations[]` or `state.intraday.openPositions[]` — Day Trading's own UI kept showing the position as open. | ✅ `markExecuted()` (~1372-1390): when the resolved account is `'trading'`, finds the matching `dayTrading.recommendations[]` entry (ticker + `status==='executed'`) and flips it to `'closed'`, and removes the matching `intraday.openPositions[]` entry by ticker. Conservative — only fires for the `'trading'` account, never blind-clears. |
| Rescan wipes executed Day Trading recs (note.txt §8 Part A cont.) | `js/day-trading-analysis.js` (~104-120) | A prior Tier-2 fix protected `status==='pending'` recs from being wiped on rescan (merge-by-ticker + `_stale` flagging), but `status==='executed'` recs weren't covered — `runDayTradeAnalysis()` still did `state.dayTrading.recommendations = newRecs` wholesale. (`runUniverseScan()` was already safe — confirmed by reading its `kept` filter.) | ✅ `runDayTradeAnalysis()` now uses the same merge-by-ticker pattern as `runUniverseScan()`: non-pending recs (executed/closed/dismissed) are always kept; stale pending recs age out after 3 missed scans as before. |
| Duplicate journal entries on close (note.txt §8 Part B) | `js/pages/recommendations.js` | Day Trading's own close functions (`closeIntradayPosition()`, `_closeDayTrade()`) correctly PATCH an existing open journal entry by `_journalId`/`recId` lookup. `markExecuted()`'s SELL/TRIM branch had no such lookup and always `unshift`ed a new entry — duplicating the record if the position was also closeable via the Day Trading path. | ✅ `markExecuted()` (~1342-1366) now looks for an existing open journal entry (ticker+account, preferring exact recId/qty match) and patches it in place instead of unshifting a duplicate, mirroring the existing Day Trading close pattern. |
| DT-specific feature fields always null (note.txt §8 Part C) | `js/pages/day-trading.js` (~1081-1087) | Snapshot builder read `rec._intradayRsi`/`rec._vwapPct`/`rec._volumeAccel` (underscore-prefixed, nonexistent), but `js/intraday-strategy.js`'s `_buildIntradayRecs()` actually sets `rec.intradayRsi`/`rec.pctFromVwap`/`rec.intradayScore` (no underscore) and never sets any volume-acceleration field at all. Three of four ML-training snapshot fields were silently null for every intraday trade ever recorded. | ✅ Fixed field reads to the real property names. Volume-acceleration field confirmed genuinely absent anywhere in the pipeline (`routes/intraday.py` only computes a boolean `vol_rising`, not a continuous metric) — left as `null` with an explanatory comment rather than inventing a fake value. |
| Intraday journal missing `parcelId` (note.txt §8 Part D) | `js/pages/day-trading.js` (~1025-1053) | The swing path (`day-trading-analysis.js` ~328-360) stamps `parcelId` on its BUY journal entry, linking it to the CGT parcel; the intraday BUY path never did, so intraday journal entries couldn't be correlated to their CGT parcel. | ✅ Added the same `newParcel` lookup + `parcelId` stamp to the intraday BUY journal entry, mirroring the swing path exactly. |
| Manual-lodge TA snapshot gap (note.txt §8 Part E) | `js/pages/journal.js` | Suspected gap in technical-data capture for manually-logged trades. | ✅ **Not a bug** — `addManualTrade()` already calls `_fetchSignalSnapshot(symbol)`, which fetches `/api/analyse/<ticker>` on-demand if the ticker isn't already in `state.liveSignals`. No fix needed. |
| #12: Broker CSV import parcel bugs (BHP/SHL/AMC) | `routes/import_csv.py` | No ticker-specific code path exists — normalization/dedup/date-parsing apply identically to all tickers. But found a real generic defect that disproportionately hits tickers with corporate-action history (BHP's 2015 South32 demerger, AMC's 2019 scheme-of-arrangement relisting, SHL's recurring bonus/SPP issues): `_parse_commsec`'s `action = "SELL" if "SELL" in desc else "BUY"` classified *any* non-SELL description as a BUY — so DRP/bonus/scheme-cancellation/off-market-transfer rows (which contain neither "BUY" nor "SELL") were fabricated into phantom BUY parcels with garbage cost bases. Same latent issue in `_parse_generic`. SelfWealth's parser was already safe (regex requires an explicit BUY/SELL token). | ✅ Added `_CORPORATE_ACTION_TOKENS` + `_is_corporate_action()`; both parsers now skip corporate-action rows (and any row matching neither BUY nor SELL) with a clear reason string instead of fabricating a phantom parcel. **Caveat:** can't be 100% certain this is *the* bug the user hit without their actual CSV — but it's a real, demonstrable defect aligned with the three cited tickers' shared trait. |
| #12: "FROM REC?" marker | `js/pages/journal.js` | The Journal table already had a binary "From Rec?" column (Yes/No/Manual) but didn't distinguish Day-Trading-sourced recs from regular AI Recommendations. | ✅ Added `_journalProvenanceBadge(t)`: checks `recId` against `state.recHistory` (regular AI), then `state.dayTrading.recommendations`/`state.intraday.recommendations` (Day Trade), else "✋ Manual" — renders `🤖 AI` / `📈 Day Trade` / `✋ Manual` badges reusing existing badge CSS classes. |
| Note.txt §9: is the ESS-based confidence correction too harsh? | `routes/learning.py` (`_calib_compute()`) | Data-driven write-up, not a code change — see below. |
| #34: DB corruption recurrence prevention | `asx_server.py`, `db.py`, `CLAUDE.md` | Suspected root cause: concurrent writers (two server instances, or an ungraceful kill mid-WAL-checkpoint) against the same SQLite file without a coordination guard. | ✅ Added a PID-based single-instance lockfile (`asx_trader.lock`, fail-open on any ambiguity — never blocks a legitimate restart on a stale/unreadable lock) in `asx_server.py`'s `__main__` bootstrap, removed via `atexit`. Added a non-blocking `PRAGMA quick_check` at startup (`db.py: quick_integrity_check()`) that logs CRITICAL + recovery guidance if corruption is detected, without refusing to start. Manually verified: second instance against the same DB is correctly refused; a stale lockfile (simulated hard-kill) is correctly reclaimed; clean shutdown removes the lockfile. Documented as CLAUDE.md gotcha #49. |

**Note.txt §9 write-up — is the -15% ESS confidence correction too harsh?**

The correction lives in `routes/learning.py`'s `_calib_compute()`: `adj_val = round(max(-0.15, min(0.10, delta * 0.8)), 2)`, where `delta = wr - mid` (actual win rate minus the confidence band's midpoint). **-15% is a hard cap, not a default** — it's only reached when `delta` is large (≥ ~-0.19, i.e. the band is overperforming-on-paper by 19+ points below its midpoint) AND the nudge clears two separate gates: `_ESS_MIN = 6.0` (Kish effective sample size, so a handful of flukes can't trigger it) and `_ci_excludes(raw_wins, n, mid)` (the Wilson 95% CI must exclude the midpoint — i.e. statistically significant, not noise). `_MIN_NUDGE_N = 12` additionally floors the raw sample size. In practice, most nudges are much smaller than the -15%/+10% caps; the caps exist to bound worst-case influence on any single Claude call, not to routinely apply a 15-point penalty.

Crucially, **the correction cannot self-launder**: `js/analysis.js` always logs the *original*, pre-correction confidence back to the Learning Loop (`ai_confidence: r._calibApplied.orig`, not the corrected value). So even if a band gets nudged down, the next calibration cycle still measures win rate against Claude's actual stated confidence, not the engine-adjusted one — the feedback loop measures truth, not its own intervention. This means the correction is self-correcting over time: if a band's real win rate moves back toward its midpoint, `delta` shrinks, the CI gate stops excluding the midpoint, and the nudge naturally fades out (gated nudges, not a ratchet).

**Verdict: not too harsh as designed** — the gating (ESS≥6, n≥12, Wilson CI exclusion) is deliberately conservative (raised from looser thresholds in Sprint 70 specifically because smaller-n nudges were firing on sampling noise), and the cap is a safety bound on a rare, statistically-significant case rather than a routine penalty. No code change recommended.
