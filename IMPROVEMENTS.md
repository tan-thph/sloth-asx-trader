...
**Last Updated:** 2026-06-20 (Sprint 72 — DB corruption incident resolved; note.txt triage of 35 user-reported issues: Tiers 1-3 (29 issues) implemented/answered and verified; Tier 4 (4 larger features, see below) scoped but not started; item #10 needs user input)
...
### Tier 4 — larger features (multi-step, needs a design conversation first)

- #12: Trade Journal parcel-tracking bugs (BHP/SHL/AMC) from broker import + distinct "FROM REC?" marker for Day-Trading trades.
- Note.txt section 8: Day Trading ↔ Trade Journal duplication on close; execution-time technical-data snapshotting for both Day Trading and manual lodges.
- Note.txt section 9: is the ESS-based -15% confidence correction too harsh, and how does it interact with Learning Loop feedback over time? (data-driven write-up, not a code change)
- #34: DB corruption recurrence prevention — process-level fix (single entrypoint discipline) once root cause is confirmed.
