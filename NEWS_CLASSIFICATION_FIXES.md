...
| **R3** | Event lifecycles | **Planned, not started** | `NEWS_EVENT_MODEL_PLAN.md`. Buildable now; payoff unproven — measure story-spam first. No `story_id`/`news_stories` exists. |
| **R7** | Historical playbooks | **R7-A shipped; R7-B/C not started** | `NEWS_EVENT_MODEL_PLAN.md`. R7-A (taxonomy `Other`→`Admin`) is done — it had to precede any playbook. R7 proper is data-starved ~2 orders of magnitude: 15 capital raises vs the critic's 82, over 24 days of history, against a 48-day median recovery. |
