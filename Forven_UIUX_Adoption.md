...
**Update (2026-06-24, same day) — §1 VALUES CORRECTION:** The implemented `terminal` theme (§1, marked Done above) was built from the GitHub repo's `app.css` only, which uses approximate/inferred colors and shipped with **no real font file** (falls back to system-mono). The actual production site (`https://forven.app/`) was inspected directly (page HTML + served CSS, not just the repo) and gives exact, verified source-of-truth values that differ from what was guessed. §1.5 below replaces the color/font specifics. Both open decisions (palette/corner fidelity, font sourcing) confirmed by the user (2026-06-24): match the live site's exact palette/corners, and source+self-host the real JetBrains Mono font file.

**Update (2026-06-24, same day) — §1.5 DONE.** All 4 action-item groups implemented in `asx_trading.css`'s `:root[data-theme="terminal"]` block:
1. Palette corrected to the live-site values (`#0a0b0d`/`#101214`/`#0d0f11`/`#1c1e20` backgrounds, `#9a9793`/`#737373` text, `#171717`/`#262626` borders, `#46d08a` up, `#f5736f` down).
2. Radius `0px` → `2px`. Pill-shaped elements (`.badge`, status dots, toggle knobs) verified to already use hardcoded `border-radius:99px`/`50%` throughout `asx_trading.css`, never a `--radius-*` token — confirmed no separate pill-radius token was needed before assuming one was.
3. New `--accent-brand: #ff6b1a` / `--accent-brand-hover: #ff8a3d`, aliased onto `--accent-primary` rather than a one-off color — `.btn-primary` and `.nav-item.active::before` (the active-nav indicator) already read `--accent-primary`, so both pick up the brand orange automatically with no separate edit.
4. Real JetBrains Mono v2.304 sourced from the official GitHub release (`github.com/JetBrains/JetBrainsMono`, SIL OFL 1.1) — **not** Google Fonts CDN. Used the release's pre-built `webfonts/JetBrainsMono-{Regular,Bold}.woff2` directly (92–95KB each, already well under the "small and subset" constraint — fonttools wasn't available in this environment to subset further, and the existing file size didn't warrant the added risk of a manual subsetting pipeline). Placed at `static/fonts/`, alongside `static/fonts/JetBrainsMono-OFL.txt` (license copy, standard SIL OFL redistribution practice — no existing third-party-asset convention was found in the repo to follow instead). Added the metric-matched `"JetBrains Mono Fallback"` `@font-face` (`local("Arial")` + ascent/descent/line-gap/size-adjust overrides) exactly as the live site does, referenced ahead of the system-mono stack.

Also folded in the "Typography details" note below: `.btn`/`.nav-label` were tracking at 0.05em/0.08em (tighter than the live site's verified .12em–.18em uppercase-micro-label range) — bumped both to 0.14em, and extended the same terminal-specific tracking to `.card-title`/`th` (which previously had no terminal override at all, just the px-based light/dark default).

Verified: WCAG contrast computed for every new color pair against the `#0a0b0d` background — all pass AA (4.5:1+) except `--text-tertiary` (4.15:1, AA-large-text tier only), which matches its existing de-emphasized/muted role elsewhere in the app, not a regression. `python test_app.py` (796/796), `npm run test:js` (154/154), CSS brace-balanced, `node --check` clean on every file.
...
This is a **warmer, slightly-blue-black** palette (`#0a0b0d`/`#101214`), not a pure `#000`/`#0a0a0a` true-black like the repo's `app.css` suggested.

**Corners — refines the "sharp corners everywhere" claim in §1:** the live site is *not* uniformly `rounded-none`. Observed radii: `2px` (most common — buttons/inputs), `3px`/`5px` (cards), `9999px` (fully-rounded pill badges/tags only), plus a few CSS-variable radii (`--radius-md/lg/xl/2xl`) on larger containers.

**DECISION (user confirmed 2026-06-24): match the live website exactly.** Action items for the worker, replacing what was already shipped:
1. In the `:root[data-theme="terminal"]` block, change `--bg-base`/`--bg-surface`/`--bg-inset`/`--bg-hover` from the pure-black guesses to `#0a0b0d` / `#101214` / `#0d0f11` / `#1c1e20` respectively (table above).
2. Change the radius override from `0px` to `2px` for the general `--radius-sm/md/lg` tokens under this theme, but add a dedicated pill/badge radius (`9999px` or reuse an existing `--radius-full`/`border-radius:50%`-style token if one already exists in `asx_trading.css` — check before adding a new one) for status badges, the ⛔ HELD badge, the 🔒 Local badge, and similar pill-shaped elements so they stay fully rounded rather than going square.
3. Add the orange accent `#ff6b1a` (base) / `#ff8a3d` (hover) as a new token (e.g. `--accent-brand`) scoped to this theme, used sparingly: the sidebar logo's accent square/dot, the active-nav-item indicator, and primary CTA buttons — not as a background fill anywhere.
4. Re-verify contrast: `#46d08a` (success) and `#f5736f`/`#fb2c36` (danger) against the new `#0a0b0d` background (should be fine — these are the live site's own pairing — but re-check via the existing accessibility-check habit applied to the other themes, if any exists).

**DECISION (user confirmed 2026-06-24): source and self-host the real font file.** Action items:
1. Download JetBrains Mono from the official source — `https://github.com/JetBrains/JetBrainsMono` releases (SIL OFL 1.1, free to redistribute, no attribution requirement in-UI) — **not** from Google Fonts' CDN (would violate the no-CDN constraint below). Take only the Latin/Latin-Extended subset (skip Cyrillic/Greek/Vietnamese subsets the live site splits out — irrelevant to this app's English-only UI, and pulling the multi-file split would bloat `static/fonts/` for no reason here).
2. Place the regular + bold weight `.woff2` files at `static/fonts/JetBrainsMono-Regular.woff2` and `static/fonts/JetBrainsMono-Bold.woff2` (variable-weight single-file is an option too, if preferred — either is fine, just keep file count/size minimal per the existing "small and subset" constraint below).
3. Add the metric-matched fallback `@font-face` exactly as the live site does — `font-family: "JetBrains Mono Fallback"; src: local(Arial); ascent-override:75.79%; descent-override:22.29%; line-gap-override:0%; size-adjust:134.59%` — and reference both in the font stack (`font-family: "JetBrains Mono", "JetBrains Mono Fallback", Consolas, "SF Mono", monospace;`) so a slow/failed font load doesn't reflow the layout, and the existing system-mono stack remains the final safety net.
4. Verify license file presence: include JetBrains Mono's `OFL.txt` alongside the font files in `static/fonts/` (standard SIL OFL redistribution practice) or reference it from a top-level `THIRD_PARTY_LICENSES` note if one exists — check if this repo already has a convention for bundled third-party assets before inventing one.
