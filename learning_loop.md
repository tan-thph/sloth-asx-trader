...
| `outcome_status`      | TEXT       | `win` / `loss` / `breakeven` / `open` / `skipped` / `expired` — or **NULL** for a freshly-logged rec (the logger doesn't send the field; it's first set by an outcome patch or `_expire_old_events`). Consumers must filter on explicit values, never `IS NOT NULL`-as-closed. Virtual values (`virtual_win`/`virtual_loss`/`virtual_open`, plus `virtual_hold_miss`/`virtual_hold_correct` for HOLDs) are stored in the separate `virtual_outcome` column. |
...
| `virtual_outcome`          | TEXT       | `virtual_win` / `virtual_loss` / `virtual_open` — lazy-resolved by `_resolve_virtual_outcomes()` for `was_executed=0` recs ≥30d old (Sprint 36). For `recommendation='HOLD'` rows the same column instead carries `virtual_hold_miss` / `virtual_hold_correct`, written by `_resolve_hold_outcomes()` (see the HOLD Outcome Tracking section) — the value sets never collide and the BUY/SELL virtual calibration query selects only `virtual_win`/`virtual_loss` |
...
| `exit_signals_json`        | TEXT       | JSON snapshot of live signals at SELL/TRIM execution time, captured by `markExecuted()` — the exit-side mirror of `entry_signals_json` (separate column, CLAUDE.md gotcha #64) |
| `exit_quality_tag`         | TEXT       | `exit_into_strength` / `exit_after_reversal_confirmed` / `exit_into_weakness` / `exit_neutral` — classified lazily by `_resolve_exit_quality_tags()` from `exit_signals_json`; distinct column, NOT folded into `error_type` |
| `regime_at_execution`      | TEXT       | Regime at execution time (write-once `regime` column keeps the generation-time value; see CLAUDE.md gotcha #65) |
| `fetch_failed_at`          | TEXT       | ISO timestamp of the last failed yfinance fetch for this row — lazy resolvers skip it for `_FETCH_RETRY_DAYS` (7d) then retry, instead of starving the capped batch or blacklisting forever |
| `calibrated_confidence`    | REAL       | POST-nudge confidence actually used for sizing; NULL when no engine-side calibration nudge fired. `ai_confidence` always stores the ORIGINAL. The pair feeds `GET /api/learning/calibration-efficacy` (did the nudge beat raw confidence on Brier?) |
| `trade_mode`               | TEXT       | `'real'` / `'paper'` (paper/real firewall, CLAUDE.md gotcha #88). Paper events get a uniform 0.5× multiplier inside `_weight()`; `real_vs_paper` stat on `/api/learning/stats` warns when the WR gap ≥15pp (n≥5 each side). In `_ALLOWED_OUTCOME_COLS` so mode is upgradeable at execution time |
| `local_scrutiny_json`      | TEXT       | Stored local-LLM scrutinize verdict for a pending rec (`POST /api/debate/scrutinize`, manual only); rolled up against eventual outcomes by `GET /api/learning/scrutiny-stats` |
...
- `calibration_snapshots` — One row per day `(snapshot_date, brier_score, n, prompt_version)`, written lazily by `GET /api/learning/calibration-stats`. Feeds `GET /api/learning/calibration-trend` (last 180d) and the Brier trend chart on the Learning page.
...
1. **Recommendation Generated** (`analysis.js` → `logRecsToLearningLoop()`)
   ...
   - **HOLD recs are logged too** — stripped from the pending/UI set but re-collected (`_holdRecs`) and appended to the log call so `_resolve_hold_outcomes()` can grade passivity (FIXES #4). Server-side dedupe: `POST /api/learning/log` returns the existing event (`deduped: true`) instead of inserting when a HOLD for the same ticker was logged within the last `_HOLD_DEDUP_DAYS` (7d) — HOLDs re-fire for every holding on every analysis run, so without the window guard event volume grows linearly with run frequency while adding no new signal
...
**HOLD outcome tracking** ✅ *(FIXES #4, July 2026)*
HOLD recs carry no stop/target (validator `requiredUnless:'HOLD'`), so `_resolve_virtual_outcomes()`'s NOT-NULL WHERE clause structurally never touches them — before this fix they were also filtered out of `logRecsToLearningLoop()` entirely, so the model was never graded on its own passivity. Now:

- **Logging:** `analysis.js` appends the stripped HOLDs (`_holdRecs`) to the log call; the backend dedupes to one HOLD event per ticker per 7 days (`_HOLD_DEDUP_DAYS`) since HOLDs re-fire on every analysis run.
- **Resolution:** `_resolve_hold_outcomes(conn, cap=5, move_threshold=0.08)` runs lazily in `_calib_compute()` for HOLD events ≥30d old. It fetches daily closes from the event date and finds the largest |% move| from the entry-day close in either direction (a HOLD commits to no direction, so a big move either way was actionable). ≥8% → `virtual_hold_miss`; otherwise `virtual_hold_correct`. Written to the existing `virtual_outcome` column, scoped to `recommendation='HOLD'` so it never perturbs the BUY/SELL virtual weighting.
- **Conventions:** the verdict **freezes at first resolution** — a `virtual_hold_correct` resolved at day 30 is not re-checked if the move comes at day 45 (same one-shot convention as the BUY/SELL virtual resolver). An empty history fetch writes `virtual_open` permanently; a *failed* fetch sets `fetch_failed_at` and retries after 7d.
- **Feedback:** `hold_outcomes {n, miss_rate, correct_rate}` on `GET /api/learning/stats`; `_compute_hold_outcome_nudge()` emits `⚠HOLD_TOO_PASSIVE(N/M HOLDs missed a >=8% move)` into the calibration block when miss rate >50% at n≥10. A low miss rate is deliberately not flagged — correct caution isn't a problem to nudge against.
...
## Decision Trackers (Sell/Trim + Buy/Top-Up) *(current session)*

Two per-trade (row-level) review tables on the Learning page, both reading exclusively from columns `ai_learning_events` already computes elsewhere in this doc — the trackers are display/aggregation layers, not new data sources. No Claude/Ollama call is made to render either.

### Why this shipped
The pre-existing **Sell Decision Tracker** (Sprint 37, see below) validated `sell_primary_driver` against a 30-day price outcome, but only showed 5 columns (date/ticker/driver/P&L/verdict) and gave no visual signal distinguishing a full **SELL** from a partial **TRIM** — both rendered identically even though `_resolve_sell_outcomes()` had always processed TRIM events too (the gap was purely in the UI, not the resolver). Separately, no per-trade view existed for the **entry** side at all — `_compute_thesis_matrix()` and `learning_factor_win_rates()` are aggregate cross-tabs (driver × verdict, factor × outcome), useful for spotting a systemic pattern but useless for reviewing "this specific BUY, what happened."

### What changed
- **`GET /api/learning/sell-outcomes`** (extended) now also returns: `recommendation` (SELL vs TRIM — the missing distinction), `holding_period_days`, `regime` + `regime_at_execution`, `sector`, `skill_score`, `exit_quality_tag`, `mae_pct`/`mfe_pct`, `thesis_verdict`, `error_type`, `bull_case`/`bear_case`. New `?action=SELL|TRIM|all` query param filters by action type; `?force=1` (as before) runs the lazy resolvers immediately rather than waiting for the next incidental `_calib_compute()` call.
- **`GET /api/learning/buy-outcomes`** (new) — the per-trade complement for entries. Same query-param shape (`limit`, `action=BUY|TOP_UP|all`, `force`). Returns `primary_entry_driver`, `thesis_verdict`, P&L/outcome, `holding_period_days`, `regime`/`regime_at_execution`, `sector`, `skill_score`, `exit_quality_tag` (present when a SELL/TRIM later cascaded it onto the parent BUY — see gotcha #64 in CLAUDE.md), `mae_pct`/`mfe_pct`, `error_type`, `bull_case`/`bear_case`. Only rows with `outcome_status IS NOT NULL` are returned (still-open positions are excluded — nothing to review yet).
- Both endpoints require zero new columns and zero new resolvers — every field returned was already being written by an existing lazy resolver (`_resolve_sell_outcomes`, `_resolve_deterministic_tags`, `_resolve_exit_quality_tags`, `_resolve_mae_mfe`) for other purposes (calibration, the Thesis Accuracy Matrix, the Success Patterns card); this feature just exposes them per-row instead of only in aggregate.

### Frontend (`js/pages/learning.js`)
- Shared helper functions (`_dtActionBadge`, `_dtThesisChip` — 5-state, per gotcha #59 — `_dtExitQualityChip` — 4-state, per gotcha #64 — `_dtSkillChip`, `_dtPnlCell`, `_dtDetailHTML`, `toggleDtRow`, `_dtFilterBar`) are used by both cards so the two trackers render and expand identically rather than duplicating markup.
- Each row shows action badge, driver, holding period, regime, P&L, exit-quality chip, skill-score chip, and (sell tracker only) the 30-day verify verdict inline; a `▸`/`▾` toggle expands a detail row with sector, both regime columns, entry/exit price, MAE/MFE, error tags, secondary factors/urgency (sell), and the bull/bear case text Claude wrote at generation time.
- Filter buttons (`setDtFilter('sell'|'buy', value)`) toggle `_sellOutcomesFilter`/`_buyOutcomesFilter` module-level state and re-render; "↺ Check Now" calls the endpoint with `force=1` then re-renders.
- Cards degrade gracefully: with the `all` filter and zero events, the whole card hides (`el.style.display='none'`); with a non-`all` filter and zero matches, the card stays visible with a "No events match this filter" message (so switching from All→SELL on a trim-only account isn't silently indistinguishable from a fetch failure).
...
| `GET /api/learning/stats`               | Read   | Aggregates for Learning Loop UI; includes `success_patterns`, `phase8_meta` (mann_whitney_z), `prompt_regression`, Wilson 95% CI on all win-rate fields, `stop_tag_precision`. *(Sprint 71)* also returns `by_sector`: per-sector `{sector, n, win_rate, avg_pnl_pct, wr_vs_market_pp, dominant_error_tag}` sorted by n desc. `wr_vs_market_pp` is the signed delta vs `overall_win_rate`; `dominant_error_tag` splits multi-tag `error_type` on `,` excluding `none`/`external_shock`. Also returns `hold_outcomes` (HOLD passivity: n/miss_rate/correct_rate), `buy_vs_topup` (per-action WR/avg-P&L/avg-hold split), `real_vs_paper` (paper-as-predictor-of-real; warns when WR gap ≥15pp, n≥5 each side), `capital_efficiency` (avg P&L per holding-day by entry driver), `ensemble_divergence` (outcomes when Claude and quant confidence disagreed), `mae_mfe_summary` — all empty-safe |
...
| `GET /api/learning/calibration-stats`   | Read   | Brier score + reliability-diagram bins `{brier_score, n, bins:[{range,lo,hi,n,mean_confidence,actual_win_rate}]}`. Also lazily writes one row/day to the `calibration_snapshots` table (feeds `calibration-trend`) |
| `GET /api/learning/calibration-trend`   | Read   | Brier-score history from `calibration_snapshots` (last 180d, chronological) — "is the loop actually learning?" Rendered as a line chart on the Learning page (Brier is lower-is-better, so declining = green) |
| `GET /api/learning/calibration-efficacy`| Read   | Does the engine-side confidence nudge beat raw confidence? Compares Brier on `ai_confidence` (original) vs `calibrated_confidence` (post-nudge) over closed win/loss recs where both exist and differ; positive `improvement` = nudges helped. n-gated |
| `GET /api/learning/recent-rec`          | Read   | `?ticker=X` → single most recent logged rec for the ticker, ANY action. Powers the **whipsaw/contradiction check** in `analysis.js`: a new rec whose direction flips within 10d of a near-identical setup (≥4/5 matching `entrySignature()` tokens, `js/utils.js`) is flagged in `_ruleWarnings` |
| `GET /api/learning/driver-matrix`       | Read   | 3D cross-tab: `primary_entry_driver` × regime × sector; only cells with n≥3 returned |
| `GET /api/learning/factor-win-rates`    | Read   | Empirical information coefficient per entry-signal factor — which captured entry signals actually discriminate wins from losses |
| `GET /api/learning/lesson-effectiveness`| Read   | Per-lesson WR before vs after the lesson's creation date, sorted worst-first — lesson lifecycle scoring. Read-only; the user prunes stale lessons manually. Rendered as chips on the Learning page Lessons card |
| `GET /api/learning/lesson-source`       | Read   | Structured dataset for the AI Lesson Generator (manual trigger): last N closed+executed trades (wins AND losses), CI-gated cross-tab aggregates, wider exemplar cap (30) than digest-data. Output is parsed into scoped `trading_lessons` rows. `?n=100` (clamped 30–200) |
| `GET /api/learning/scrutiny-stats`      | Read   | Rollup of local-LLM scrutinize verdicts (`local_scrutiny_json`) vs eventual outcomes — the local critic's track record, fed to the AI Lesson Generator |
| `GET /api/learning/go-live-readiness`   | Read   | Paper-validation scorecard — is the track record strong enough to fund live trading? Five friction-adjusted criteria (sample size ≥40, regime coverage ≥3, calibration honesty = \|stated conf − actual WR\| ≤7pp, edge survives a `?friction_bps=N` haircut with Wilson-CI-gated win-rate > payoff-ratio break-even, ≥2 entry drivers with positive post-cost expectancy at n≥5). Returns `{overall: ready\|almost\|not_ready, n_pass, criteria[]}`. Renders as the 🎯 Go-Live Readiness card at the top of the Learning page. **Uses the calibration GAP, not Brier** — Brier has an irreducible p(1−p) floor that would punish an honest 65%-win model |
| `GET /api/learning/coverage`            | Read   | Data-hygiene coverage strip: per-capture fill rates over closed trades (`error_type`, `success_tags`, `entry_driver`, `thesis_verdict`, `mae_mfe`, `exit_signals` — each `{covered,total,pct}`, `pct=null` on empty denominator) + work queues (`stuck_open_60d`, `virtual_pending`/`days_until_virtual`, `hold_events`/`hold_resolved`, `lessons`). Read-only COUNTs; triggers no lazy resolvers. Renders as the 🧹 Capture Coverage card at the top of the Learning page *(IMPROVEMENTS #3)* |
